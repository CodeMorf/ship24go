...
import { generateId, hashPassword, UserRepo } from '../db/repos';

let pointSchemaReady = false;

async function ensurePointSchema() {
  if (pointSchemaReady) return;
...
function randomRef(prefix: string, country = '') {
  const day = new Date().toISOString().slice(2, 10).replace(/-/g, '');
  const token = crypto.randomBytes(4).toString('hex').toUpperCase();
  return [prefix, country ? upperCode(country, 3) : '', day, token].filter(Boolean).join('-');
}
...
function publicItem(row: any) {
  const sender = safeJson(row.sender_json, {});
  const recipient = safeJson(row.recipient_json, {});
  return {
    id: row.id,
...
async function addPointEvent(itemId: string, eventCode: string, status: string, label: string, location = '', metadata: any = null) {
  await pool.query(
...
async function queueSnapshot(point: any, itemType: string, destinationCountry = 'DO') {
  ...
  const ageDays = oldest ? Math.max(0, Math.floor((Date.now() - oldest.getTime()) / 86400000)) : 0;
  ...
  return {
    ...
    minItems,
    minDays,
    eligible: qty >= minItems || ageDays >= minDays,
  };
}

async function evaluateQueue(point: any, itemType: string, destinationCountry = 'DO', force = false) {
  const snapshot = await queueSnapshot(point, itemType, destinationCountry);
  if (!snapshot.qty) return { released: false, reason: 'empty', ...snapshot };
  if (!snapshot.eligible && !force) return { released: false, reason: 'threshold', ...snapshot };
  if (point.decision_mode === 'approval' && !force) return { released: false, reason: 'approval', recommended: true, ...snapshot };

  const hubId = snapshot.rule?.hub_id || point.hub_id;
  if (!hubId) return { released: false, reason: 'hub', ...snapshot };

  const [hubRows]: any = await pool.query('SELECT * FROM point_hubs WHERE id = ? AND status = \'active\' LIMIT 1', [hubId]);
  const hub = hubRows?.[0];
  if (!hub) return { released: false, reason: 'hub', ...snapshot };
  ...
  if (!items?.length) return { released: false, reason: 'empty', ...snapshot };

  const manifestId = generateId('pmf_');
  const tracking = randomRef('MNF', hub.code || point.country);
  ...
  try {
    ...
    await conn.query(
      `INSERT INTO point_manifests (id, point_id, hub_id, manifest_tracking, item_type, item_count, status)
       VALUES (?, ?, ?, ?, ?, ?, 'ready')`,
      [manifestId, point.id, hub.id, tracking, itemType, items.length]
    );
    const ids = items.map((item: any) => item.id);
    const placeholders = ids.map(() => '?').join(',');
    await conn.query(
      `UPDATE point_items SET manifest_id = ?, hub_id = ?, status = 'consolidated', status_label = 'Consolidado para traslado al hub'
       WHERE id IN (${placeholders})`,
      [manifestId, hub.id, ...ids]
    );
    for (const item of items) {
      await conn.query(
        `INSERT INTO point_tracking_events (id, item_id, event_code, status, label, location, metadata_json)
         VALUES (?, ?, 'manifest_ready', 'consolidated', 'Consolidado para traslado al hub', ?, ?)`,
        [generateId('pte_'), item.id, cleanText(`${hub.name}${hub.city ? ` · ${hub.city}` : ''}`, 191), JSON.stringify({ manifestId, manifestTracking: tracking })]
      );
    }
    await conn.commit();
  } catch (error) {
  ...
  return {
    released: true,
    manifest: { id: manifestId, tracking, itemCount: items.length, hubId: hub.id, hubName: hub.name },
    ...snapshot,
  };
}

async function availableCommission(pointId: string) {
  const [commissionRows]: any = await pool.query(
    `SELECT COALESCE(SUM(CASE WHEN status IN ('earned','held') THEN amount ELSE 0 END),0) AS earned,
  ...
  const earned = money(commissionRows?.[0]?.earned);
  const committed = money(payoutRows?.[0]?.committed);
  return {
    earned,
    settled: money(commissionRows?.[0]?.settled),
    committed,
    paid: money(payoutRows?.[0]?.paid),
    available: money(Math.max(0, earned - committed)),
  };
}
  ...
  return async (req: any, res: any, next: any) => {
    try {
      ...
      if (req.user?.role === 'super_admin') {
        ...
        const [rows]: any = await pool.query('SELECT p.*, h.name AS hub_name, h.code AS hub_code FROM partner_points p LEFT JOIN point_hubs h ON h.id = p.hub_id WHERE p.id = ? LIMIT 1', [pointId]);
...
export const createPointRouter = (authMiddleware: any) => {
  ...
  router.get('/public/tracking/:code', async (req, res) => {
    try {
      ...
      const status = finalDelivered ? 'delivered' : item.status;
      const statusLabel = finalDelivered ? 'Entregado' : (item.final_status_label || item.status_label);
  ...
  router.get('/me', async (req: any, res) => {
    ...
    const payoutDetails = safeJson(point.payout_details_json, {});
    res.json({
      ...
      payoutProfile: { method: point.payout_method || 'manual', details: payoutDetails },
    });
  });
    ...
    try {
      ...
      const [manifests]: any = await pool.query(
        `SELECT m.*, h.name AS hub_name FROM point_manifests m JOIN point_hubs h ON h.id = m.hub_id
         WHERE m.point_id = ? ORDER BY m.created_at DESC LIMIT 8`, [point.id]
      );
      ...
      const commission = await availableCommission(point.id);
      const queue: Record<string, any> = {};
      for (const type of ['document', 'card', 'envelope']) {
        const q = await queueSnapshot(point, type, 'DO');
        queue[type] = { qty: q.qty, ageDays: q.ageDays, minItems: q.minItems, minDays: q.minDays, eligible: q.eligible, approvalRequired: point.decision_mode === 'approval' };
      }
      const stats = statsRows?.[0] || {};
      res.json({
        point: publicPoint(point),
        summary: { total: Number(stats.total || 0), today: Number(stats.today || 0), queued: Number(stats.queued || 0), transit: Number(stats.transit || 0), delivered: Number(stats.delivered || 0) },
        commission,
        queue,
  ...
  router.post('/items', async (req: any, res) => {
    ...
    try {
      ...
      if (!cleanText(sender.name, 191) || !cleanText(recipient.name, 191)) return res.status(400).json({ error: 'Completa el nombre del remitente y del destinatario.' });
      if (!cleanText(recipient.phone || recipient.email, 191)) return res.status(400).json({ error: 'Agrega un teléfono o correo del destinatario.' });

      const destinationCountry = upperCode(req.body?.destinationCountry || recipient.country || 'DO', 2) || 'DO';
      const priceMap: any = { document: point.document_price, card: point.card_price, envelope: point.envelope_price, parcel: point.parcel_price };
      const commissionMap: any = { document: point.document_commission, card: point.card_commission, envelope: point.envelope_commission, parcel: point.parcel_commission };
      const servicePrice = money(priceMap[itemType]);
      const commissionAmount = money(commissionMap[itemType]);
      ...
      await conn.query(
        ...
        [itemId, point.id, req.user.id, itemType, tracking, receipt, JSON.stringify(sender), JSON.stringify(recipient), cleanText(req.body?.description, 255) || null,
          money(req.body?.declaredValue), servicePrice, commissionAmount, point.currency || 'EUR', destinationCountry, hubId]
      );
      await conn.query(
        `INSERT INTO point_commissions (id, point_id, item_id, amount, currency, status) VALUES (?, ?, ?, ?, ?, 'earned')`,
        [generateId('pcm_'), point.id, itemId, commissionAmount, point.currency || 'EUR']
      );
      await conn.query(
        `INSERT INTO point_tracking_events (id, item_id, event_code, status, label, location)
         VALUES (?, ?, 'point_received', 'queued', 'Recibido en Point autorizado', ?)`,
        [generateId('pte_'), itemId, cleanText(`${point.name}${point.city ? ` · ${point.city}` : ''}`, 191)]
      );
      await conn.commit();

      const release = await evaluateQueue(point, itemType, destinationCountry, false);
      ...
      res.status(201).json({
        ...
        item: publicItem(createdRows?.[0]),
        receipt: { number: receipt, tracking, price: servicePrice, currency: point.currency || 'EUR', pointName: point.name, createdAt: createdRows?.[0]?.created_at },
      ...
      res.status(500).json({ error: 'No se pudo crear la emisión. Intenta nuevamente.' });
    } finally {
      conn.release();
    }
  });
  ...
  router.get('/payouts', async (req: any, res) => {
    try {
      ...
      res.json({ payouts: rows || [], commission: await availableCommission(req.point.id) });
    } catch (error) {
  ...
  router.post('/payouts', async (req: any, res) => {
    try {
      ...
      const balances = await availableCommission(req.point.id);
      if (amount <= 0) return res.status(400).json({ error: 'Ingresa un monto válido.' });
      if (amount > balances.available) return res.status(400).json({ error: 'El monto supera las comisiones disponibles.' });
      const method = ['bank', 'paypal', 'manual'].includes(String(req.point.payout_method)) ? req.point.payout_method : 'manual';
      const initialStatus = req.point.payout_approval_mode === 'automatic' ? 'approved' : 'pending';
      const payoutId = generateId('ppy_');
      await pool.query(
        `INSERT INTO point_payout_requests (id, point_id, amount, currency, method, status, payout_details_json, requested_by, approved_at)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
        [payoutId, req.point.id, amount, req.point.currency || 'EUR', method, initialStatus, req.point.payout_details_json || null, req.user.id, initialStatus === 'approved' ? new Date() : null]
      );
      res.status(201).json({ success: true, message: initialStatus === 'approved' ? 'Solicitud aprobada según la configuración del Point.' : 'Solicitud de pago registrada correctamente.', payoutId, status: initialStatus });
    } catch (error) {
...
function adminPointGuard(requireAdminOrPermission: any) {
...
async function adminBootstrap() {
  const [points]: any = await pool.query(
    `SELECT p.*, h.name AS hub_name, h.code AS hub_code,
            COUNT(DISTINCT po.user_id) AS operators,
            COUNT(DISTINCT i.id) AS item_count,
            COALESCE(SUM(DISTINCT CASE WHEN c.status IN ('earned','held') THEN c.amount ELSE 0 END),0) AS earned_commission
     FROM partner_points p
     LEFT JOIN point_hubs h ON h.id = p.hub_id
     LEFT JOIN point_operators po ON po.point_id = p.id AND po.active = 1
     LEFT JOIN point_items i ON i.point_id = p.id
     LEFT JOIN point_commissions c ON c.point_id = p.id
  ...
  const [manifests]: any = await pool.query(
    `SELECT m.*, p.name AS point_name, h.name AS hub_name
     FROM point_manifests m JOIN partner_points p ON p.id = m.point_id JOIN point_hubs h ON h.id = m.hub_id
     ORDER BY m.created_at DESC LIMIT 100`
  );
  const [payouts]: any = await pool.query(
    `SELECT pr.*, p.name AS point_name FROM point_payout_requests pr JOIN partner_points p ON p.id = pr.point_id
  ...
  const [summaryRows]: any = await pool.query(
    `SELECT (SELECT COUNT(*) FROM partner_points WHERE status = 'active') AS active_points,
            ...
            (SELECT COALESCE(SUM(amount),0) FROM point_commissions WHERE status IN ('earned','held')) AS commissions`
  );
  return {
    summary: summaryRows?.[0] || {},
    points: (points || []).map((p: any) => ({ ...publicPoint(p), operators: Number(p.operators || 0), itemCount: Number(p.item_count || 0), earnedCommission: money(p.earned_commission) })),
    hubs: hubs || [], rules: rules || [], manifests: manifests || [], payouts: payouts || [],
  };
}

export const createAdminPointsRouter = (authMiddleware: any, requireAdminOrPermission: any) => {
  const router = Router();
  router.use(authMiddleware, adminPointGuard(requireAdminOrPermission));
  router.use(async (_req, res, next) => {
    try { await ensurePointSchema(); next(); }
    catch (error) { console.error('[Point Admin] initialization error', error); res.status(500).json({ error: 'No se pudo abrir la gestión de Points.' }); }
  });

  router.get('/bootstrap', async (_req, res) => {
    try { res.json(await adminBootstrap()); }
    catch (error) { console.error('[Point Admin] bootstrap error', error); res.status(500).json({ error: 'No se pudo cargar la red de Points.' }); }
  });
  ...
  router.post('/hubs', async (req: any, res) => {
    try {
      ...
      await pool.query(
        `INSERT INTO point_hubs (id, code, name, country, city, address, postal_code, status) VALUES (?, ?, ?, ?, ?, ?, ?, 'active')`,
  ...
  router.post('/points', async (req: any, res) => {
    ...
    try {
      ...
      const operatorName = cleanText(req.body?.operatorName || name, 191);
      const operatorPassword = String(req.body?.operatorPassword || '');
      ...
      await conn.query(
        `INSERT INTO partner_points
         ...
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'active', ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
        [pointId, code, name, cleanText(req.body?.legalName, 191) || null, country, currency, cleanText(req.body?.city, 120) || null,
          cleanText(req.body?.address, 255) || null, cleanText(req.body?.postalCode, 24) || null, cleanText(req.body?.phone, 60) || null,
          cleanText(req.body?.email, 191) || operatorEmail || null, cleanText(req.body?.hubId, 36) || null,
          ['approval', 'rules'].includes(req.body?.decisionMode) ? req.body.decisionMode : 'rules', money(req.body?.documentPrice), money(req.body?.cardPrice),
          money(req.body?.envelopePrice), money(req.body?.parcelPrice), money(req.body?.documentCommission), money(req.body?.cardCommission),
          money(req.body?.envelopeCommission), money(req.body?.parcelCommission), req.body?.parcelEnabled ? 1 : 0,
      ...
      if (operatorEmail) {
        const existing = await UserRepo.getByEmail(operatorEmail);
        if (existing) {
          if (existing.role === 'super_admin') throw new Error('ADMIN_OPERATOR_NOT_ALLOWED');
          operatorId = existing.id;
          await conn.query(`UPDATE users SET role = 'point_operator', status = 'active', business_type = 'Ship24Go Point' WHERE id = ?`, [operatorId]);
        } else {
          if (operatorPassword.length < 8) throw new Error('POINT_PASSWORD_REQUIRED');
          operatorId = generateId('usr_');
          await conn.query(
            ...
            [operatorId, operatorEmail, hashPassword(operatorPassword), operatorName, cleanText(req.body?.phone, 50), country, currency]
          );
        }
        await conn.query(`INSERT INTO point_operators (point_id, user_id, operator_role, active) VALUES (?, ?, 'owner', 1)`, [pointId, operatorId]);
      }
      await conn.commit();
      ...
      if (error?.message === 'ADMIN_OPERATOR_NOT_ALLOWED') return res.status(400).json({ error: 'Selecciona otra cuenta para operar este Point.' });
      res.status(500).json({ error: error?.code === 'ER_DUP_ENTRY' ? 'Ya existe un Point o una cuenta con esos datos.' : 'No se pudo crear el Point.' });
    } finally {
      conn.release();
    }
  });
  ...
  router.put('/points/:id', async (req: any, res) => {
    try {
      ...
      for (const [key, column] of Object.entries(allowed)) {
        ...
        sets.push(`${column} = ?`); values.push(value === '' ? null : value);
      }
  ...
  router.post('/points/:id/operators', async (req: any, res) => {
    ...
    try {
      ...
      const operatorRole = ['owner','manager','clerk'].includes(req.body?.operatorRole) ? req.body.operatorRole : 'clerk';
      if (!email || !name) return res.status(400).json({ error: 'Completa el nombre y correo del operador.' });
      ...
      if (!user) {
        ...
        await conn.query(
          `INSERT INTO users (id, email, password_hash, name, phone, country, currency, role, business_type, balance, status)
           VALUES (?, ?, ?, ?, ?, ?, ?, 'point_operator', 'Ship24Go Point', 0, 'active')`,
          [userId, email, hashPassword(password), name, cleanText(req.body?.phone, 50), point.country, point.currency]
        );
      } else if (user.role === 'super_admin') {
        throw new Error('ADMIN_OPERATOR_NOT_ALLOWED');
      } else {
        await conn.query(`UPDATE users SET role = 'point_operator', status = 'active' WHERE id = ?`, [userId]);
      }
      await conn.query(
        `INSERT INTO point_operators (point_id, user_id, operator_role, active) VALUES (?, ?, ?, 1)
  ...
  router.post('/rules', async (req: any, res) => {
    try {
      ...
      await pool.query(
        `INSERT INTO point_routing_rules (id, point_id, origin_country, destination_country, item_type, min_items, min_days, hub_id, priority, active)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 1)`,
        [id, cleanText(req.body?.pointId, 36) || null, upperCode(req.body?.originCountry, 2) || null, upperCode(req.body?.destinationCountry || 'DO', 2) || 'DO', itemType,
          Math.max(1, Number(req.body?.minItems || 1)), Math.max(0, Number(req.body?.minDays || 1)), hubId, Math.max(1, Number(req.body?.priority || 100))]
      );
  ...
  router.put('/rules/:id', async (req: any, res) => {
    try {
      await pool.query(
        ...
        [Math.max(1, Number(req.body?.minItems || 1)), Math.max(0, Number(req.body?.minDays || 0)), Math.max(1, Number(req.body?.priority || 100)), req.body?.active === false ? 0 : 1, cleanText(req.body?.hubId, 36), req.params.id]
      );
  ...
  router.post('/queue/release', async (req: any, res) => {
    try {
      ...
      const itemType = cleanText(req.body?.itemType, 20).toLowerCase();
      if (!['document','card','envelope','parcel'].includes(itemType)) return res.status(400).json({ error: 'Selecciona un tipo de envío válido.' });
      const result = await evaluateQueue(point, itemType, upperCode(req.body?.destinationCountry || 'DO', 2) || 'DO', true);
  ...
  async function manifestStep(req: any, res: any, step: 'dispatch'|'receive'|'export') {
    try {
      const [rows]: any = await pool.query(
        `SELECT m.*, h.name AS hub_name, h.city AS hub_city, h.country AS hub_country FROM point_manifests m JOIN point_hubs h ON h.id = m.hub_id WHERE m.id = ? LIMIT 1`,
        [req.params.id]
      );
      const manifest = rows?.[0];
      if (!manifest) return res.status(404).json({ error: 'Manifiesto no encontrado.' });
      const mapping: any = {
        dispatch: { manifestStatus: 'in_transit_to_hub', itemStatus: 'in_transit_to_hub', label: 'En tránsito hacia el hub', event: 'to_hub', date: 'dispatched_at' },
        receive: { manifestStatus: 'at_hub', itemStatus: 'at_hub', label: 'Recibido en hub', event: 'hub_received', date: 'received_at' },
        export: { manifestStatus: 'exported', itemStatus: 'exported', label: 'Salida internacional confirmada', event: 'international_departure', date: 'exported_at' },
      };
      const m = mapping[step];
      const extra: string[] = [];
      const values: any[] = [m.manifestStatus];
      ...
      values.push(req.params.id);
      await pool.query(`UPDATE point_manifests SET status = ?, ${m.date} = NOW()${extra.length ? `, ${extra.join(', ')}` : ''} WHERE id = ?`, values);
      const [items]: any = await pool.query('SELECT id FROM point_items WHERE manifest_id = ?', [req.params.id]);
      await pool.query(`UPDATE point_items SET status = ?, status_label = ? WHERE manifest_id = ?`, [m.itemStatus, m.label, req.params.id]);
      for (const item of items || []) await addPointEvent(item.id, m.event, m.itemStatus, m.label, step === 'receive' || step === 'export' ? `${manifest.hub_name}${manifest.hub_city ? ` · ${manifest.hub_city}` : ''}` : '');
      res.json({ success: true, message: m.label });
    } catch (error) {
  ...
  router.post('/items/:id/final-delivery', async (req: any, res) => {
    try {
      ...
      let finalStatus = 'final_mile';
      let finalLabel = 'Asignado a entrega final';
      let finalTracking = cleanText(req.body?.finalShipmentTracking, 191) || null;
      if (shipmentId) {
        const [shipRows]: any = await pool.query('SELECT id, tracking_code, status, status_label FROM shipments WHERE id = ? LIMIT 1', [shipmentId]);
        const ship = shipRows?.[0];
        if (!ship) return res.status(400).json({ error: 'Selecciona un envío final válido.' });
        finalTracking = finalTracking || ship.tracking_code || null;
        if (['entregado','delivered'].includes(String(ship.status || '').toLowerCase())) { finalStatus = 'delivered'; finalLabel = 'Entregado'; }
      }
      await pool.query(
        `UPDATE point_items SET final_shipment_id = ?, final_shipment_tracking = ?, local_tracking = ?, status = ?, status_label = ?, delivered_at = ? WHERE id = ?`,
        [shipmentId, finalTracking, cleanText(req.body?.localTracking, 191) || null, finalStatus, finalLabel, finalStatus === 'delivered' ? new Date() : null, item.id]
      );
      await addPointEvent(item.id, 'final_delivery_linked', finalStatus, finalLabel, cleanText(req.body?.location, 191));
      res.json({ success: true, message: finalStatus === 'delivered' ? 'Entrega final confirmada.' : 'Entrega final asociada correctamente.' });
    } catch (error) {
  ...
  router.post('/payouts/:id/approve', async (req: any, res) => {
    try {
      const [result]: any = await pool.query(
        `UPDATE point_payout_requests SET status = 'paid', approved_by = ?, approved_at = NOW(), paid_at = NOW(), note = ?
  ...
  router.post('/payouts/:id/reject', async (req: any, res) => {
    try {
      const [result]: any = await pool.query(
        `UPDATE point_payout_requests SET status = 'rejected', approved_by = ?, approved_at = NOW(), note = ? WHERE id = ? AND status IN ('pending','approved')`,
        [req.user.id, cleanText(req.body?.note, 500) || null, req.params.id]
      );
    ...
    try {
      const [rows]: any = await pool.query(
        `SELECT i.*, p.name AS point_name, m.manifest_tracking, m.status AS manifest_status, h.name AS hub_name
         FROM point_items i JOIN partner_points p ON p.id = i.point_id
         LEFT JOIN point_manifests m ON m.id = i.manifest_id LEFT JOIN point_hubs h ON h.id = i.hub_id
         WHERE i.customer_tracking = ? OR i.receipt_number = ? LIMIT 1`,
        [cleanText(req.params.code, 80).toUpperCase(), cleanText(req.params.code, 80).toUpperCase()]
      );
      ...
      res.json({ item: publicItem(rows[0]), operation: { pointName: rows[0].point_name, manifestTracking: rows[0].manifest_tracking, manifestStatus: rows[0].manifest_status, hubName: rows[0].hub_name, localTracking: rows[0].local_tracking, finalShipmentId: rows[0].final_shipment_id, finalShipmentTracking: rows[0].final_shipment_tracking }, events: events || [] });
    } catch (error) {
  ...
  router.get('/bootstrap/refresh', async (_req, res) => {
    try { res.json(await adminBootstrap()); }
    catch (error) { console.error('[Point Admin] refresh error', error); res.status(500).json({ error: 'No se pudo actualizar la información.' }); }
  });
