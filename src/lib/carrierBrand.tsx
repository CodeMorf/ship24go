...
export function resolveCarrierName(quote: any): string {
  ...
  for (const candidate of candidates) {
    ...
    if (n.includes('genei') || n.includes('parcel') || n.includes('spedire') || n.includes('spediamo') || n.includes('pacco') || n.includes('easypost') || n.includes('ship24')) continue;
...
const BRAND_COLORS: Record<string, string> = {
  ...
  Logihub: 'from-blue-600 to-indigo-700',
};

function monogram(name: string) {
  const parts = String(name || 'LH').trim().split(/\s+/).filter(Boolean);
  if (parts.length >= 2) return (parts[0][0] + parts[1][0]).toUpperCase();
  return String(name || 'LH').slice(0, 2).toUpperCase();
}
