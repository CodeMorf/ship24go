...
const SERVICE_FILTERS = [
  { key: 'all', label: 'Todas', hint: 'Mejor precio y tiempo' },
  { key: 'economy', label: 'Económico', hint: 'Ahorro primero' },
  { key: 'fast', label: 'Rápido', hint: 'Menor tiempo' },
  { key: 'dropoff', label: 'Oficina / punto', hint: 'Entrega en punto' },
  { key: 'door', label: 'A domicilio', hint: 'Recogida o entrega directa' },
];
...
const quoteMatchesFilter = (quote: any, filter: string) => {
  if (filter === 'all') return true;
  const profile = getQuoteProfile(quote);
  const text = normalizeText(`${quote?.service || ''} ${quote?.collectionTypeName || ''}`);
  if (filter === 'dropoff') return quoteNeedsAnyPoint(quote) || profile.key === 'dropoff' || text.includes('drop-off') || text.includes('dropoff') || text.includes('point') || text.includes('hub');
  if (filter === 'door') return !quoteNeedsAnyPoint(quote) || profile.key === 'door' || text.includes('door') || text.includes('home') || text.includes('pickup') || text.includes('pick-up');
...
const getQuoteServiceBadges = (quote: any) => {
  const { departure, arrival } = quotePointTypes(quote);
  const badges: { label: string; className: string }[] = [];
  if (departure === 'point') badges.push({ label: 'Punto de recogida', className: 'text-violet-700 bg-violet-50 border-violet-100 dark:text-violet-300 dark:bg-violet-950/20 dark:border-violet-900/30' });
  if (arrival === 'point') badges.push({ label: 'Punto de entrega', className: 'text-cyan-700 bg-cyan-50 border-cyan-100 dark:text-cyan-300 dark:bg-cyan-950/20 dark:border-cyan-900/30' });
  if (departure !== 'point') badges.push({ label: 'Retiro a domicilio', className: 'text-blue-700 bg-blue-50 border-blue-100 dark:text-blue-300 dark:bg-blue-950/20 dark:border-blue-900/30' });
  if (arrival !== 'point') badges.push({ label: 'Entrega a domicilio', className: 'text-emerald-700 bg-emerald-50 border-emerald-100 dark:text-emerald-300 dark:bg-emerald-950/20 dark:border-emerald-900/30' });
  const text = normalizeText(`${quote?.service || ''} ${quote?.collectionTypeName || ''} ${quote?.providerPayload?.name || ''}`);
  if (text.includes('locker') || text.includes('inpost')) badges.push({ label: 'Locker disponible', className: 'text-amber-700 bg-amber-50 border-amber-100 dark:text-amber-300 dark:bg-amber-950/20 dark:border-amber-900/30' });
  return badges;
};
...
const Quote = () => {
  ...
  const handleQuote = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const phaseTimer = window.setTimeout(() => setLoadingPhase(t('quote_loading_phase3') || 'Comparando precios y plazos...'), 1800);
      ...
      const nextQuotes = (Array.isArray(res?.quotes) ? res.quotes : []).map((q: any) => ({
        ...q,
      ...
      if (nextQuotes.length === 0) {
        setError(res?.message || res?.error || t('quote_no_options') || 'No encontramos opciones disponibles para esta ruta.');
      }
  ...
  const visibleQuotes = quotes.filter((quote: any) => quoteMatchesFilter(quote, serviceFilter));
    ...
    <div className="py-2 md:py-4 max-w-5xl mx-auto">
      ...
      {quotes.length > 0 && !loading && (
        <div className="space-y-4 animate-in fade-in slide-in-from-bottom-4 duration-300">
          <h2 className="text-sm font-bold text-gray-400 uppercase tracking-wider mb-2">{(t('availableOptions') || 'Opciones Disponibles')} · {quotes.length}</h2>
          {visibleQuotes.map((q: any) => (
            <div key={q.id} className="glass-panel p-6 rounded-2xl border border-gray-200 dark:border-gray-800 flex flex-col md:flex-row items-center justify-between hover:border-blue-400 dark:hover:border-neon-cyan transition-all group" style={{ borderColor: q.providerColor ? `${q.providerColor}33` : undefined }}>
              <div className="flex flex-col sm:flex-row items-center gap-4 flex-1 mb-4 md:mb-0 w-full">
                {/* Logo del Transportista */}
                <div className="flex-shrink-0">
                  {q.providerLogo ? (
                    <img src={q.providerLogo} alt={getCarrierName(q)} className="w-12 h-12 rounded-xl object-contain bg-white border border-gray-100 shadow-sm p-1" />
                  ) : getCarrierLogo(getCarrierName(q), q.service)}
                </div>
                
                <div className="flex-1 w-full text-center sm:text-left">
                  <div className="flex flex-wrap items-center justify-center sm:justify-start gap-3 mb-1">
                     <p className="text-xl font-black text-gray-900 dark:text-white group-hover:text-blue-600 dark:group-hover:text-neon-cyan transition-colors">{getCarrierName(q)}</p>
                  </div>
                  <p className="text-sm text-gray-500 dark:text-gray-400 flex items-center justify-center sm:justify-start gap-1">
                     <i className="fa-regular fa-clock"></i> {t('estimatedDelivery')}: <strong className="text-gray-700 dark:text-gray-300">{q.deliveryText || `${q.estimatedDays} ${t('days')}`}</strong>
                  </p>
                  <p className="mt-1 text-xs font-semibold text-gray-500 dark:text-gray-400">
                    Servicio: <span className="text-gray-800 dark:text-gray-200">{cleanCustomerServiceName(q)}</span>
                  </p>
                  
                  {/* Badges of Type and Insurance */}
                  <div className="flex flex-wrap items-center justify-center sm:justify-start gap-2 mt-2">
                    <span className={`inline-flex items-center text-[10px] font-bold px-2 py-0.5 rounded-md border gap-1 shadow-sm ${getQuoteProfile(q).className}`}>
                      <Sparkles className="w-3.5 h-3.5" /> {getQuoteProfile(q).label}
                    </span>
                    <span className="inline-flex items-center text-[10px] font-bold text-blue-600 dark:text-[#66FCF1] bg-blue-50 dark:bg-blue-950/20 px-2 py-0.5 rounded-md border border-blue-100 dark:border-blue-900/30 gap-1 shadow-sm">
                      <Truck className="w-3.5 h-3.5" /> {getCollectionLabel(q)}
                    </span>
                    {getQuoteServiceBadges(q).map((badge, index) => (
                      <span key={`${badge.label}-${index}`} className={`inline-flex items-center text-[10px] font-bold px-2 py-0.5 rounded-md border gap-1 shadow-sm ${badge.className}`}>
                        <MapPin className="w-3.5 h-3.5" /> {badge.label}
                      </span>
                    ))}
                    <span className="inline-flex items-center text-[10px] font-bold text-emerald-600 dark:text-emerald-400 bg-emerald-50 dark:bg-emerald-950/20 px-2 py-0.5 rounded-md border border-emerald-100 dark:border-emerald-900/30 gap-1 shadow-sm">
                      <ShieldCheck className="w-3.5 h-3.5" /> Precio protegido
                    </span>
                  </div>
                </div>
              </div>
              <div className="flex items-center gap-6 w-full md:w-auto justify-between md:justify-end border-t md:border-t-0 border-gray-100 dark:border-gray-800 pt-4 md:pt-0 mt-4 md:mt-0">
                <div className="text-right">
                  {(q.serviceTypeLabel || q.deliveryText || q.estimatedDaysMin) ? (
                    <p className="text-[10px] text-sky-600 dark:text-sky-400 font-bold uppercase tracking-wide mb-1">
                      {[q.serviceTypeLabel, q.deliveryText || (q.estimatedDaysMin && q.estimatedDaysMax ? (String(q.estimatedDaysMin) + '-' + String(q.estimatedDaysMax) + ' ' + (t('days_unit') || t('days') || 'dias')) : '')].filter(Boolean).join(' | ')}
                    </p>
                  ) : null}
                  <p className="text-xs text-gray-400 dark:text-gray-500 font-medium uppercase tracking-wider mb-1">Precio Final</p>
          ...
          {visibleQuotes.length === 0 && (
            <div className="glass-panel p-6 rounded-2xl border border-gray-200 dark:border-gray-800 text-sm font-bold text-gray-500 dark:text-gray-400">
              No hay opciones para este filtro. Prueba con “Todas” o busca otra ruta.
            </div>
          )}
        </div>
      )}
...
const CUSTOMER_SERVICE_TERMS: Record<string, { title: string; body: string[] }> = {
  es: {
    ...
    body: [
      ...
      'Si necesita asegurar un envío por un valor superior al máximo permitido o si necesita asegurar un artículo prohibido o con limitaciones que quiere enviar de forma regular, por favor póngase en contacto con soporte@ship24go.com y estudiaremos la posibilidad de crear un seguro especial para usted.'
    ]
  },
  en: {
    ...
    body: [
      ...
      'If you need to insure a shipment above the permitted maximum, or need recurring coverage for a prohibited or restricted item, please contact soporte@ship24go.com and we will review the possibility of creating special insurance for you.'
    ]
  },
  it: {
    ...
    body: [
      ...
      'Se devi assicurare una spedizione per un valore superiore al massimo consentito, o vuoi spedire regolarmente un articolo proibito o con limitazioni, contatta soporte@ship24go.com e valuteremo la possibilità di creare un’assicurazione speciale per te.'
    ]
  }
};
...
const getCustomerShipmentStatus = (shipment: any) => {
  ...
  if (cancelStatus === 'refunded') {
    return {
      ...
      helper: 'El importe fue acreditado en tu monedero Ship24Go.',
      Icon: CheckCircle,
        ...
        <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white dark:bg-dark-900 rounded-3xl border border-gray-200 dark:border-gray-800 shadow-2xl w-full max-w-3xl max-h-[88vh] overflow-y-auto">
            <div className="p-6 border-b border-gray-100 dark:border-gray-800 flex items-start justify-between gap-4">
              <div>
                <p className="text-xs font-black uppercase tracking-[0.2em] text-blue-600 dark:text-neon-cyan">Ship24Go</p>
          ...
          <div className="glass-panel p-4 sm:p-6 md:p-8 rounded-3xl border border-gray-200 dark:border-gray-800 flex flex-col justify-between">
              ...
              <div className="pt-6 border-t border-gray-100 dark:border-gray-800 mt-6 flex items-center justify-between">
                  <div>
                      ...
                      <p className="text-sm font-black text-pink-500 dark:text-neon-pink uppercase tracking-wide">{String((profile as any)?.subscription?.planName || (profile as any)?.currentPlanName || 'Ship24Go Básico').toUpperCase()} ACTIVO</p>
                  </div>
                  <button 
...
export default function CustomerPanel() {
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-dark-900 font-sans flex flex-col transition-colors">
      {adminBackupToken && (
        <div className="bg-slate-900 text-white px-4 py-2 text-sm font-bold flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 z-[60]">
      ...
      <Header toggleTheme={toggleTheme} isDark={isDark} toggleMobileMenu={toggleMobileMenu} profile={profile} />
      <div className="flex flex-1 overflow-hidden relative">
        <Sidebar isMobileMenuOpen={isMobileMenuOpen} toggleMobileMenu={toggleMobileMenu} profile={profile} />
