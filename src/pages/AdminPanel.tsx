...
const AdminSidebar = ({ isMobileMenuOpen, toggleMobileMenu, currentUser, isDark, toggleTheme }: any) => {
  ...
  const ALL_MENU_ITEMS = [
    { name: t('dashboard'), path: '/admin', icon: LayoutDashboard, permission: 'dashboard.view' },
    { name: 'Cotizar & Crear', path: '/admin/quote', icon: PlusCircle, permission: 'shipments.view' },
    { name: t('clients'), path: '/admin/clients', icon: Users, permission: 'clients.view' },
    { name: t('totalShipments'), path: '/admin/shipments', icon: Package, permission: 'shipments.view' },
    { name: t('providers'), path: '/admin/providers', icon: Truck, permission: 'providers.view' },
    { name: 'Integraciones', path: '/admin/integraciones', icon: Plug, permission: 'integrations.manage' },
    { name: 'Polar', path: '/admin/polar', icon: Sparkles, permission: 'payments.manage' },
    { name: 'PayPal', path: '/admin/paypal', icon: CreditCard, permission: 'payments.manage' },
    ...
    { name: t('ai_copilot'), path: '/admin/copilot', icon: Sparkles, permission: 'copilot.use' },
  ];
  ...
  const menu = ALL_MENU_ITEMS.filter(item => hasPermission(item.permission));

  return (
    <>
      {isMobileMenuOpen && (
        <div onClick={toggleMobileMenu} className="fixed inset-0 bg-slate-950/75 backdrop-blur-xs z-50 md:hidden animate-fade-in"></div>
      )}
      <aside className={`fixed md:static inset-y-0 left-0 transform ${isMobileMenuOpen ? 'translate-x-0' : '-translate-x-full'} md:translate-x-0 transition-transform duration-300 w-72 max-w-[85vw] bg-slate-900 text-white min-h-screen flex flex-col font-sans z-50 shadow-2xl md:shadow-none pb-6`}>
  ...
  const handleSave = async () => {
    ...
    try {
      const payload = providers.map((provider) => ({
        ...provider,
        margin_percent: Number(provider.margin ?? provider.margin_percent ?? 20),
              ...
              <div key={`${p.code}-${i}`} className="bg-white rounded-3xl border border-gray-100 shadow-sm overflow-hidden">
                <div className="p-6 border-b border-gray-100 flex items-start justify-between gap-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      />
                      <p className="text-xs text-gray-400 font-bold uppercase tracking-wider">{p.code}</p>
                    </div>
                  </div>
                  <label className="flex items-center gap-2 text-xs font-bold text-gray-600 select-none">
                    <input type="checkbox" checked={Boolean(p.is_active)} onChange={(e) => updateProvider(i, 'is_active', e.target.checked)} className="rounded text-blue-600" />
                    Activo
                  </label>
                </div>
                ...
                <div className="p-6 space-y-5">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    {p.code === 'easypost' && (
                      <>
                        ...
                        <div>
                          ...
                          <input type="text" value={cfg.webhookUrl || 'https://ship24go.com/api/webhooks/easypost'} onChange={(e) => updateProviderConfig(i, 'webhookUrl', e.target.value)} className="w-full px-3 py-2 bg-gray-50 border border-gray-200 rounded-xl text-gray-900 text-sm font-semibold outline-none focus:border-blue-400" />
                        </div>
                    ...
                    <div>
                      ...
                      <input
                        type="number"
                        min="0"
                        value={p.margin ?? p.margin_percent ?? 20}
                  ...
                  <div className="flex flex-wrap items-center justify-between gap-3 pt-4 border-t border-gray-100">
                    <div>
                      <span className={`inline-flex items-center px-3 py-1 rounded-full text-xs font-bold ${p.is_connected ? 'bg-green-100 text-green-700' : p.is_active ? 'bg-yellow-100 text-yellow-700' : 'bg-gray-100 text-gray-500'}`}>
                        {p.is_connected ? 'Integrado' : p.is_active ? 'Pendiente' : 'No disponible'}
                      </span>
                      {p.balance?.label && (
                    ...
                    <div className="flex flex-wrap gap-2">
                      <button onClick={() => handleTestConnection(p)} disabled={testingMap[p.code]} className="px-4 py-2 bg-blue-50 text-blue-600 hover:bg-blue-100 disabled:bg-gray-100 disabled:text-gray-400 font-bold rounded-lg text-xs transition-colors">
                        {testingMap[p.code] ? 'Probando...' : p.code === 'paccofacile' ? 'Validar Paccofacile' : p.code === 'spedirepro' ? 'Validar SpedirePro' : p.code === 'spediamopro' ? 'Validar SpediamoPro' : p.code === 'easypost' ? 'Validar EasyPost' : 'Probar conexión'}
                      </button>
                      {p.code === 'easypost' && (
...
const AdminPlans = () => {
  ...
  const handleChange = (index: number, field: string, value: any) => {
    setPlans((current) => current.map((plan, i) => {
      ...
      if (['price', 'discount'].includes(field)) return { ...plan, [field]: Number(value || 0) };
          ...
          {plans.map((plan, i) => (
            <div key={plan.id} className="bg-white p-6 rounded-3xl border border-gray-100 shadow-sm flex flex-col gap-5">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div><label className="block text-xs font-bold text-gray-500 uppercase tracking-wider mb-2">Descuento %</label><input type="number" step="0.01" value={plan.discount || 0} onChange={(e) => handleChange(i, 'discount', e.target.value)} className="w-full px-4 py-2 bg-gray-50 border border-gray-200 rounded-lg font-bold text-gray-900 focus:outline-none focus:border-blue-500" /></div>
      ...
      <div className="rounded-3xl bg-white dark:bg-dark-800 border border-slate-100 dark:border-slate-800 shadow-sm overflow-hidden">
        ...
        {loading ? <div className="p-12 text-center text-slate-500">Cargando información...</div> : !status?.recent?.length ? <div className="p-12 text-center text-slate-500">No hay registros todavía.</div> : (
          <div className="overflow-x-auto"><table className="w-full text-left">
            ...
            <tbody className="divide-y divide-slate-100 dark:divide-slate-800">{status.recent.map((ev: any) => <tr key={ev.id}><td className="p-4 font-mono font-bold text-blue-600 dark:text-neon-cyan">{ev.trackingCode || ev.shipmentId}</td><td className="p-4"><span className="rounded-full bg-slate-100 dark:bg-dark-900 px-3 py-1 text-xs font-black">{ev.statusLabel}</span></td><td className="p-4 text-sm font-bold uppercase">{ev.providerCode || 'Ship24Go'}</td><td className="p-4 text-xs text-slate-500">{ev.eventTime ? new Date(ev.eventTime).toLocaleString() : ''}</td><td className="p-4 text-sm text-slate-500 dark:text-slate-400">{ev.description}</td></tr>)}</tbody>
          </table></div>
        )}
      </div>
    </div>
...
const AdminIntegrations = () => {
  ...
  const copyRedirect = async () => {
    const value = status?.redirectUrl || 'https://ship24go.com/api/integrations/ecartapi/callback';
    try {
          ...
          <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
            <div className="xl:col-span-2 bg-white dark:bg-dark-950 rounded-3xl border border-slate-100 dark:border-slate-800 p-6 shadow-sm">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-8">
                ...
                <div className="md:col-span-2 rounded-2xl bg-slate-50 dark:bg-dark-900 p-4 border border-slate-100 dark:border-slate-800">
                  ...
                  <div className="mt-2 flex flex-col md:flex-row gap-3">
                    <input readOnly value={status?.redirectUrl || 'https://ship24go.com/api/integrations/ecartapi/callback'} className="flex-1 px-4 py-3 rounded-xl bg-white dark:bg-dark-950 border border-slate-200 dark:border-slate-800 font-mono text-sm text-slate-900 dark:text-slate-100" />
              ...
              <div className="mt-6 rounded-2xl bg-amber-50 border border-amber-100 p-4 text-sm text-amber-900 font-semibold">
                Para producción, esta URL debe estar configurada en el dashboard de Ecart. La conexión de tiendas es gratuita para el cliente; Ship24Go monetiza la generación de etiquetas y la gestión logística.
              </div>
            </div>
            ...
            <div className="bg-white dark:bg-dark-950 rounded-3xl border border-slate-100 dark:border-slate-800 p-6 shadow-sm">
              ...
              <div className="space-y-3">
                {[
                  ...
                  'Ship24Go devuelve tracking a la tienda.'
                ].map((item, index) => (
...
const AdminPaymentIntegration = ({ provider }: { provider: 'polar' | 'paypal' }) => {
  ...
  const copyWebhook = async () => {
    const url = data.settings?.webhookUrl || data.status?.webhookUrl || `https://ship24go.com/api/webhooks/${provider}`;
  ...
  return (
    <div className="p-4 sm:p-6 md:p-8 space-y-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div className="rounded-3xl bg-white border border-gray-100 p-6 shadow-sm"><p className="text-xs font-black text-gray-500 uppercase tracking-wider">Webhook</p><p className={`text-2xl font-black mt-3 ${webhookReady ? 'text-emerald-600' : 'text-amber-600'}`}>{webhookReady ? copy.configured : copy.pending}</p><p className="text-sm text-gray-500 mt-2">{data.settings?.webhookUrl || `https://ship24go.com/api/webhooks/${provider}`}</p></div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-8">
        <div className="rounded-3xl bg-white border border-gray-100 p-6 shadow-sm space-y-5">
          ...
          <div><label className="block text-xs font-black text-gray-500 uppercase mb-2">{copy.webhookUrl}</label><div className="flex flex-col sm:flex-row gap-3"><input readOnly value={data.settings?.webhookUrl || `https://ship24go.com/api/webhooks/${provider}`} className="flex-1 px-4 py-3 rounded-xl bg-gray-50 border border-gray-200 font-mono text-xs" /><button onClick={copyWebhook} className="px-5 py-3 rounded-xl bg-gray-900 text-white font-bold">{copy.copy}</button></div></div>
...
const AdminSettings = () => {
  ...
  const [apiKeys, setApiKeys] = useState<Record<string, string>>({
    ...
    ecartApiClientId: '',
    ecartClientSecret: '',
    ecartAppUrl: 'https://ship24go.com',
  ...
  const polarWebhookUrl = apiKeys.polarWebhookUrl || 'https://ship24go.com/api/webhooks/polar';
  const polarEnvironment = apiKeys.polarEnvironment || 'sandbox';
  const paypalWebhookUrl = apiKeys.paypalWebhookUrl || 'https://ship24go.com/api/webhooks/paypal';
  ...
  const handleCopySpedireWebhook = async () => {
    const value = spedireInfo?.webhookUrl || 'https://ship24go.com/api/webhooks/spedirepro';
    try {
      await navigator.clipboard.writeText(value);
    ...
    <div className="p-4 sm:p-6 md:p-8">
      <div className="flex flex-col sm:flex-row justify-between sm:items-center gap-4 mb-8">
        ...
        <div className="flex flex-wrap gap-3">
          ...
          <Link to="/admin/settings/docs" className="bg-indigo-600 hover:bg-indigo-700 text-white px-6 py-2.5 rounded-xl font-bold flex items-center gap-2 transition-colors shadow-lg self-start sm:self-auto">
            <BookOpen className="w-4 h-4" /> Docs & Arquitectura
          </Link>
          <Link to="/admin/settings/audit" className="bg-rose-600 hover:bg-rose-700 text-white px-6 py-2.5 rounded-xl font-bold flex items-center gap-2 transition-colors shadow-lg self-start sm:self-auto">
            <ShieldAlert className="w-4 h-4" /> Auditoría & Logs
          </Link>
      ...
      <div className="bg-white p-6 rounded-2xl border border-violet-100 shadow-sm mb-8">
        <div className="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-5 mb-6">
          <div>
            ...
            <p className="text-gray-500 text-sm max-w-2xl">Pega esta URL en SpedirePro para recibir creación de envío, actualizaciones y resultado de recogida. Ship24Go guardará el estado y buscará la etiqueta si todavía no está disponible.</p>
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 mb-6">
          <div className="lg:col-span-2 bg-slate-50 border border-slate-100 rounded-2xl p-4">
            ...
            <div className="flex flex-col sm:flex-row gap-2">
              <input readOnly value={spedireInfo?.webhookUrl || 'https://ship24go.com/api/webhooks/spedirepro'} className="flex-1 px-4 py-3 bg-white border border-slate-200 rounded-xl font-mono text-sm text-slate-900" />
            ...
            <p className="text-xs text-slate-500 mt-2">Tracking público: {spedireInfo?.trackingBaseUrl || 'https://ship24go.com/tracking?code='}</p>
          </div>
...
export default function AdminPanel() {
  ...
  useEffect(() => {
    ...
    api.getProfile()
      .then((res: any) => {
        const user = res.user;
        const isStaff = user && (user.role === 'super_admin' || user.role !== 'customer' || (user.permissions && user.permissions.length > 0));
        if (!isStaff) {
          removeAuthToken();
  ...
  return (
    <div className="flex flex-col min-h-screen bg-slate-50 dark:bg-dark-900 font-sans transition-colors relative">
      ...
      <div className="flex flex-1 overflow-hidden relative flex-col md:flex-row">
        ...
        />
        <main className="flex-1 overflow-y-auto h-[calc(100vh-4rem)] md:h-screen bg-slate-50 dark:bg-dark-900 transition-colors pb-20 md:pb-6">
          <Routes>
      ...
      {/* Barra de Navegación Inferior Móvil (Sticky Bottom Bar) */}
      <nav className="md:hidden fixed bottom-0 left-0 right-0 h-16 bg-slate-900/95 backdrop-blur-md border-t border-slate-800 flex items-center justify-around px-2 z-40 shadow-lg">
        <Link 
          to="/admin" 
          className={`flex flex-col items-center justify-center py-1.5 px-3 rounded-xl text-[10px] font-bold transition-colors ${location.pathname === '/admin' ? 'text-blue-400' : 'text-slate-400 hover:text-white'}`}
        >
        ...
        <Link 
          to="/admin/shipments" 
          className={`flex flex-col items-center justify-center py-1.5 px-3 rounded-xl text-[10px] font-bold transition-colors ${location.pathname.startsWith('/admin/shipments') ? 'text-blue-400' : 'text-slate-400 hover:text-white'}`}
        >
        ...
        <Link 
          to="/admin/team" 
          className={`flex flex-col items-center justify-center py-1.5 px-3 rounded-xl text-[10px] font-bold transition-colors ${location.pathname.startsWith('/admin/team') ? 'text-blue-400' : 'text-slate-400 hover:text-white'}`}
        >
        ...
        <Link 
          to="/admin/clients" 
          className={`flex flex-col items-center justify-center py-1.5 px-3 rounded-xl text-[10px] font-bold transition-colors ${location.pathname.startsWith('/admin/clients') ? 'text-blue-400' : 'text-slate-400 hover:text-white'}`}
        >
        ...
        <button 
          ...
          className="flex flex-col items-center justify-center py-1.5 px-3 rounded-xl text-[10px] font-bold text-slate-400 hover:text-white transition-colors cursor-pointer"
        >
