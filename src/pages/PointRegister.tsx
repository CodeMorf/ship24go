...
import { ArrowRight, Building2, CheckCircle2, ChevronLeft, MapPin, ShieldCheck, Store, User, XCircle } from 'lucide-react';
...
function MapPreview({ latitude, longitude }: { latitude: number | null; longitude: number | null }) {
  ...
  if (!ready) {
    return (
      <div className="h-full min-h-[250px] rounded-3xl border border-dashed border-slate-300 bg-slate-50 flex flex-col items-center justify-center text-center p-8">
        <MapPin className="w-10 h-10 text-slate-300 mb-3" />
        <p className="font-bold text-slate-600">La ubicación aparecerá aquí</p>
        <p className="text-sm text-slate-400 mt-1 max-w-xs">Selecciona una sugerencia de Google Maps para guardar la coordenada exacta del comercio.</p>
      </div>
    );
  }

  return (
    <div className="relative h-full min-h-[250px] rounded-3xl overflow-hidden border border-slate-200 bg-slate-100">
      <div ref={mapRef} className="absolute inset-0" />
      <div className="absolute bottom-3 left-3 right-3 rounded-2xl bg-white/95 backdrop-blur px-4 py-3 shadow-lg border border-white">
        <p className="text-[10px] font-black uppercase tracking-wider text-blue-600">Coordenadas verificadas</p>
        <p className="font-mono text-xs text-slate-700 mt-1">{Number(latitude).toFixed(7)}, {Number(longitude).toFixed(7)}</p>
      </div>
    </div>
  );
}
    ...
    <div className="min-h-screen bg-slate-950 text-slate-900 font-sans">
      <header className="border-b border-white/10 bg-slate-950/95 sticky top-0 z-20">
        <div className="max-w-7xl mx-auto px-5 sm:px-8 py-4 flex items-center justify-between">
          <Link to="/"><BrandMark dark iconClassName="w-9 h-9 rounded-xl" textClassName="text-xl text-white" /></Link>
          <Link to="/auth/login" className="text-sm font-bold text-slate-300 hover:text-white">Ya tengo una cuenta</Link>
        </div>
      </header>
      ...
      <main className="max-w-7xl mx-auto px-5 sm:px-8 py-10 lg:py-16">
        <div className="grid lg:grid-cols-[0.86fr_1.14fr] gap-8 lg:gap-12 items-start">
          <section className="text-white lg:sticky lg:top-28">
            <div className="inline-flex items-center gap-2 rounded-full border border-cyan-400/30 bg-cyan-400/10 px-3 py-1.5 text-xs font-black uppercase tracking-wider text-cyan-300">
            ...
            <p className="text-slate-400 text-lg leading-relaxed mt-6 max-w-xl">Registra tu ubicación, recibe operaciones y consulta el mismo tracking que verá el cliente final.</p>
            <div className="mt-10 space-y-4">
              {[
                ...
                <div key={title} className="flex gap-4 rounded-2xl border border-white/10 bg-white/[0.04] p-4">
                  <div className="w-10 h-10 shrink-0 rounded-xl bg-cyan-400/10 text-cyan-300 flex items-center justify-center"><Icon className="w-5 h-5" /></div>
                  <div><p className="font-black">{title}</p><p className="text-sm text-slate-400 mt-1 leading-relaxed">{text}</p></div>
                </div>
              ))}
            </div>
          </section>

          <form onSubmit={submit} className="bg-white rounded-[2rem] shadow-2xl p-5 sm:p-8 space-y-7">
            <div className="flex items-start justify-between gap-4">
              <div><p className="text-xs font-black uppercase tracking-wider text-blue-600">Solicitud de afiliación</p><h2 className="text-2xl font-black text-slate-900 mt-1">Datos del Point</h2><p className="text-sm text-slate-500 mt-1">Completa los datos reales del comercio.</p></div>
              <div className="hidden sm:flex w-11 h-11 rounded-2xl bg-blue-50 text-blue-600 items-center justify-center"><Building2 className="w-5 h-5" /></div>
            </div>
            ...
            <div className="border-t border-slate-100 pt-6 space-y-4">
              <div><h3 className="font-black text-slate-900">Ubicación del comercio</h3><p className="text-xs text-slate-500 mt-1">Selecciona una sugerencia; escribir texto libre no genera coordenadas verificadas.</p></div>
              <div className="grid sm:grid-cols-2 gap-4">
              ...
              {locationSelected && <div className="rounded-2xl border border-emerald-200 bg-emerald-50 px-4 py-3 text-sm text-emerald-800"><p className="font-black flex items-center gap-2"><CheckCircle2 className="w-4 h-4" /> Ubicación confirmada por Google Maps</p><p className="font-mono text-xs mt-1">{Number(form.latitude).toFixed(7)}, {Number(form.longitude).toFixed(7)}</p></div>}
            </div>
            ...
            <div className="grid md:grid-cols-2 gap-4 border-t border-slate-100 pt-6">
              ...
              <div className="rounded-3xl bg-slate-50 border border-slate-200 p-5 flex flex-col justify-between"><div><p className="text-xs font-black uppercase tracking-wider text-slate-400">Dirección normalizada</p><p className="font-bold text-slate-800 mt-2 leading-relaxed">{form.formattedAddress || 'Selecciona la ubicación para confirmar la dirección.'}</p></div><div className="mt-6 text-xs text-slate-500 space-y-2"><p><span className="font-bold">Place ID:</span> {form.googlePlaceId ? `${form.googlePlaceId.slice(0, 18)}…` : 'pendiente'}</p><p><span className="font-bold">Estado:</span> {locationSelected ? 'Listo para revisión' : 'Falta seleccionar en Maps'}</p></div></div>
            </div>
            ...
            {!locationSelected && <p className="text-center text-xs font-semibold text-amber-700">Selecciona primero una dirección de Google Maps para continuar.</p>}
            <div className="flex items-center justify-between text-sm"><Link to="/" className="text-slate-500 hover:text-slate-800 flex items-center gap-1 font-bold"><ChevronLeft className="w-4 h-4" /> Volver</Link><span className="text-slate-400">Revisión manual antes de operar</span></div>
          </form>
        </div>
      </main>
    </div>
