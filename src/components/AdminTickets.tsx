...
export function AdminTickets() {
  ...
  const handleSendReply = async (e?: React.FormEvent, customMsg?: string) => {
    ...
    try {
      ...
      const senderName = customMsg && customMsg.includes('AI') ? 'Ship24go AI Assistant' : 'Super Admin';
  ...
  const handleApproveCancellation = async () => {
    ...
    const note = window.prompt('Nota para el cliente:', 'Solicitud aprobada. El importe fue acreditado en tu monedero Ship24Go.');
    if (note === null) return;
    try {
              ...
              <div className="bg-gradient-to-r from-blue-50 via-indigo-50 to-purple-50 p-6 rounded-3xl border border-blue-100 shadow-sm relative overflow-hidden">
                ...
                <div className="flex items-center justify-between mb-4 relative z-10">
                  <h3 className="font-bold text-gray-900 flex items-center gap-2">
                    ...
                    <span>Copiloto de Soporte Ship24go AI</span>
                  </h3>
                  <button
                    ...
                    <div key={reply.id} className={`flex gap-3 max-w-[85%] ${isAdmin ? 'ml-auto flex-row-reverse' : ''}`}>
                      ...
                      <div className={`p-4 rounded-2xl border ${
                        ...
                        <p className={`text-xs font-bold mb-1 ${
                          ...
                          {reply.senderName || (isAI ? 'Asistente IA Ship24go' : 'Administrador')}
                        </p>
