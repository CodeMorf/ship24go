...
app.get('/api/public/countries', async (req, res) => {
  ...
  try {
    ...
    let rows: any[] = [];
    try {
      // Use same DB helper patterns as rest of server
      // @ts-ignore
      const mod = await import('./server/db/connection');
      // @ts-ignore
      let pool: any = (mod as any).pool || (mod as any).default || (mod as any).getPool;
      if (typeof pool === 'function') pool = pool();
      if (pool && typeof pool.then === 'function') pool = await pool;
      if (!pool || !pool.query) {
        // fallback: mysql2 createPool from env
...
app.get('/manifest.json', async (req, res) => {
  try {
    ...
    res.json({
      short_name: brand.shortName || brand.siteName || 'Ship24go',
      name: brand.siteName || 'Ship24go',
...
function ship24goPolarWebhookUrl() {
  return `${process.env.APP_URL || 'https://ship24go.com'}/api/webhooks/polar`;
}
...
async function getGeneiToken(credentials: string): Promise<string> {
  ...
  try {
    const response = await fetch('https://apiv2.genei.es/api/v2/login', {
      method: 'POST',
...
function buildGeneiV1Packages(packages: any[], customs?: any[], content?: string, declaredValue?: number) {
  // Genei array_bultos: one physical parcel per entry (qty expanded). Do NOT start with [[]] junk.
  const rows: any[] = [];
...
async function callGeneiV1(provider: any, keys: any, fn: string, payload: any) {
  ...
  const response = await fetch(`${geneiV1BaseUrl(provider)}/${fn}`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
...
async function callPaccofacile(provider: any, keys: any, method: string, pathName: string, body?: any) {
  ...
  const response = await fetch(paccofacileEndpoint(provider, pathName), {
    method,
    headers,
...
async function callSpedirePro(provider: any, keys: any, method: string, pathName: string, body?: any) {
  ...
  const response = await fetch(spedireProEndpoint(provider, pathName), {
    method,
...
function buildSpedireProAddress(addr: any, quote: any, fallbackCountry: any, fallbackZip: any, providerEmail: string) {
  ...
  return {
    name: compactText(safeAddressName(addr, 'Ship24Go Cliente'), 27),
...
async function getSpediamoProToken(provider: any, keys: any) {
  ...
  const response = await fetch(`${spediamoProBaseUrl(provider)}/auth/token`, {
    method: 'POST',
    headers: {
...
async function callSpediamoPro(provider: any, keys: any, method: string, pathName: string, body?: any) {
  const credentials = spediamoProCredentials(provider, keys);
...
function buildSpediamoProShipmentContact(addr: any, quote: any, fallbackCountry: any, fallbackZip: any, providerEmail: string) {
  ...
  return {
    name: compactText(safeAddressName(source, 'Ship24Go Cliente'), 35),
...
function spediamoProCourierName(courier: any) {
  ...
  return compactText(courier || 'Logihub', 42);
}
...
async function callEasyPost(provider: any, keys: any, method: string, pathName: string, body?: any) {
  if (!easyPostCredentials(provider, keys)) return { httpStatus: 0, response: { message: 'Proveedor no disponible todavía.' }, request: null };
  const response = await fetch(`${easyPostBaseUrl(provider)}/${String(pathName || '').replace(/^\/+/, '')}`, {
    method,
...
function easyPostCarrierName(rateOrShipment: any) {
  ...
  return compactText(code || 'Logihub', 42);
}
...
function extractGeneiPointTypes(offer: any) {
  const text = normalizeProviderText([
    offer?.nombre_completo_agencia,
    offer?.nombre_agencia,
    offer?.service_name,
    offer?.name,
    offer?.collection_type,
    offer?.collectionTypeName,
    offer?.nombre_integracion_cliente,
    offer?.descripcion,
    offer?.description,
    offer?.tipo_servicio,
    offer?.service_type
  ].filter(Boolean).join(' '));
  const pickupPoint = text.includes('punto a punto') || text.includes('point to point') || text.includes('punto-punto') || text.includes('oficina-oficina') || text.includes('shop2shop') || text.includes('drop off') || text.includes('drop-off') || text.includes('dropoff') || text.includes('locker') || text.includes('inpost') || text.includes('mondial') || text.includes('delegacion') || text.includes('delegación') || text.includes('oficina') || Number(offer?.id_oficina_salida || 0) > 0;
  const deliveryPoint = pickupPoint || text.includes('entrega en punto') || text.includes('entrega oficina') || text.includes('entregar delegacion') || text.includes('entregar delegación') || text.includes('disponible en oficina') || Number(offer?.id_oficina_entrega || 0) > 0;
...
async function searchGeneiDropPoints(provider: any, keys: any, params: any) {
  ...
  // --- C) Fallback multi-carrier network via ParcelABC / SpedirePro for same country (commercial Logihub network) ---
  try {
    const keys = await ApiKeysRepo.get();
    // ParcelABC token
...
async function ensureSpedireProProviderSeed() {
  try {
    ...
    await ProviderRepo.updateAll([{ id: 'prv_spedirepro', code: 'spedirepro', name: 'SpedirePro', is_active: true, is_connected: false, margin: 20, currency: 'EUR', config: { baseUrl: process.env.SPEDIREPRO_BASE_URL || 'https://www.spedirepro.com/public-api', apiKey: process.env.SPEDIREPRO_API_KEY || '', allowRealBuy: false, publicName: 'Logihub', primaryColor: '#7c3aed', secondaryColor: '#a855f7', maxResults: 10, priority: 25 } }]);
...
async function ensureEasyPostProviderSeed() {
  try {
    ...
    if (current) return current;
    await ProviderRepo.updateAll([{ id: 'prv_easypost', code: 'easypost', name: 'EasyPost', is_active: true, is_connected: false, margin: 20, currency: 'USD', config: { mode: process.env.EASYPOST_MODE || 'test', baseUrl: process.env.EASYPOST_BASE_URL || 'https://api.easypost.com/v2', apiKey: process.env.EASYPOST_TEST_API_KEY || process.env.EASYPOST_API_KEY || '', allowRealBuy: false, allowTestLabels: true, publicName: 'Logihub', primaryColor: '#111827', secondaryColor: '#2563eb', maxResults: 10, priority: 21, webhookUrl: `${process.env.APP_URL || 'https://ship24go.com'}/api/webhooks/easypost` } }]);
...
function paccofacileSandboxTestPayload(serviceId: any, pickupLocality: any, destinationLocality: any) {
  return {
    ...
    pickup: {
      ...
      header_name: 'Ship24Go Test',
      address: 'Via Roma',
      building_number: '11',
      phone: '3331234567',
      email: ship24goProviderContactEmail(),
      note: 'Test sandbox Ship24Go'
    },
    destination: {
      iso_code: 'IT',
      postal_code: '00135',
...
async function lookupPaccofacileLocality(provider: any, keys: any, country: string, postalCode: string, cityHint?: string) {
  ...
  for (const candidate of candidates) {
    ...
    const { httpStatus, response } = await callPaccofacile(provider, keys, 'POST', 'service/locality/validation', payload);
...
function buildPaccofacileAddress(addr: any, quote: any, locality: any, kind: 'pickup' | 'destination', providerEmail: string) {
  ...
  return {
    ...
    header_name: compactText(safeAddressName(addr, kind === 'pickup' ? 'Ship24Go Sender' : 'Ship24Go Client'), 80),
...
function appBaseUrl() {
  return String(process.env.APP_URL || 'https://ship24go.com').replace(/\/$/, '');
}
...
async function sendShipmentStatusEmail(shipment: any, statusCode: string, statusLabel: string, description?: string) {
  ...
  const brand = settings?.brand || {};
  const brandName = brand.siteName || 'Ship24go';
  const senderName = process.env.MAIL_FROM_NAME || process.env.BREVO_FROM_NAME || 'Ship24go';
...
const CANCELLATION_EMAIL_COPY: any = {
  es: {
    ...
    approved_body: 'Hemos aprobado la solicitud y acreditamos el importe en tu monedero Ship24Go.',
  ...
  en: {
    ...
    approved_body: 'We approved the request and credited the amount to your Ship24Go wallet.',
  ...
  it: {
    ...
    approved_body: 'Abbiamo approvato la richiesta e accreditato l’importo sul tuo portafoglio Ship24Go.',
  ...
  fr: {
    ...
    approved_body: 'Nous avons approuvé la demande et crédité le montant sur votre portefeuille Ship24Go.',
...
async function sendCancellationEmail(shipment: any, cancellation: any, eventCode: string, options: any = {}) {
  ...
  const payload = { sender: { name: senderName, email: senderEmail }, to: [{ email: toEmail, name: isInternal ? 'Ship24Go Operaciones' : (user?.name || undefined) }], subject, htmlContent, tags: ['cancellation', eventCode] };
...
function providerFallbackEmail(...values: any[]) {
  ...
  return process.env.SHIP24GO_DEFAULT_CONTACT_EMAIL || process.env.PUBLIC_SUPPORT_EMAIL || 'help@ship24go.com';
}

function ship24goProviderContactEmail() {
  return isValidEmailForProvider(process.env.SHIP24GO_PROVIDER_CONTACT_EMAIL || process.env.PROVIDER_CONTACT_EMAIL || 'help@ship24go.com') || 'help@ship24go.com';
}
...
function calculateProviderPrice(provider: any, providerCost: number) {
  const cfg = providerConfig(provider);
  const marginPercent = Number(provider?.margin_percent ?? cfg.marginPercent ?? 20);
...
async function callProviderCreate(args: {
  ...
  if (String(providerCode || '').toLowerCase() === 'logihub_intl') {
    ...
    if (!credentials) {
      result.errorMessage = 'LogiHub no configurado. Añade LOGiHUB_API_KEY o apiKey en el proveedor.';
      return result;
    }
    if (!credentials.allowRealBuy) {
      result.errorMessage = 'LogiHub conectado. Activa allowRealBuy para emitir etiquetas reales.';
    ...
    if (!created.response?.ok && created.httpStatus >= 400) {
      result.errorMessage = created.response?.message || 'No se pudo crear el envío en LogiHub. Revisa saldo wallet LogiHub y datos del destinatario.';
  ...
  if (providerCode === 'genei') {
    ...
    if (geneiApiMode(dbProvider) === 'v1') {
      ...
      const requestPayload: any = {
        ...
        nombre_salida: safeAddressName(senderForProvider, 'Ship24Go Operaciones'),
    ...
    const appUrl = (process.env.APP_URL || 'https://ship24go.com').replace(/\/$/, '');
...
// ========== LOGIHUB INTERNACIONAL (DO → mundo) V1 ==========
...
function logihubIntlConfig(provider: any = null) {
  ...
  return {
    ...
    publicName: String(cfg.publicName || 'Logihub'),
...
function publicProviderName(provider: any): string {
  const cfg = providerConfig(provider);
  return String(cfg.publicName || cfg.displayName || 'Logihub');
}
...
const requireAdminOrPermission = (permission?: string) => {
  return (req: any, res: any, next: any) => {
    ...
    if (req.user.role !== 'customer' && req.user.status === 'active') {
      ...
      if (req.user.permissions && (req.user.permissions.includes('*') || req.user.permissions.includes(permission))) {
        return next();
      }
    }
...
registerModularRoutes(app, authMiddleware, requireAdminOrPermission);
...
app.get(['/docs', '/docs/', '/api/docs', '/api/docs/'], (req, res) => {
  ...
  res.type('html').send(swaggerUiHtml({ openapiUrl: '/openapi.json', title: 'Ship24Go API — Docs', embed }));
});
...
app.get('/api/docs/pdf', (req, res) => {
  try {
    ...
    const filename = `Ship24Go-API-Docs-${doc.lang.toUpperCase()}.pdf`;
  ...
  try {
    ...
    const [subscriptionRows]: any = await pool.query(
      `SELECT
          ...
          COALESCE(pl.name, 'Ship24Go Básico') AS planName,
...
function ecartDefaultRedirectUrl() {
  return `${String(process.env.APP_URL || 'https://ship24go.com').replace(/\/$/, '')}/api/integrations/ecartapi/callback`;
}
...
async function getEcartConfig() {
  ...
  const appUrl = String(process.env.ECARTAPI_APP_URL || keys?.ecartAppUrl || process.env.APP_URL || 'https://ship24go.com').replace(/\/$/, '');
...
async function pushEcartFulfillmentForOrder(orderId: string, userId: string) {
  ...
  const trackingUrl = row.track_url || `${String(process.env.APP_URL || 'https://ship24go.com').replace(/\/$/, '')}/tracking?code=${encodeURIComponent(trackingCode || '')}`;
  const body = {
    fulfillment: {
      fulfillmentOrderId,
      tracking: { number: trackingCode, company: inferCarrierName(row.provider_code || 'Logihub'), url: trackingUrl },
      notifyCustomer: true
    }
  };
...
async function ship24goEnsurePolarWalletProduct(keys: any, polarToken: string, environment: string) {
  ...
  const payload = {
    name: 'Ship24Go Wallet Recarga',
    description: 'Recarga segura de saldo Ship24Go.',
    visibility: 'public',
...
app.post('/api/user/recharge', authMiddleware, async (req: any, res) => {
  try {
    ...
    const appUrl = process.env.APP_URL || appBaseUrl() || 'https://ship24go.com';
...
app.post('/api/user/recharge', authMiddleware, async (req: any, res) => {
  try {
    ...
    const appUrl = process.env.APP_URL || 'https://ship24go.com';
    ...
    for (const serviceLevel of levels) {
      ...
      try {
        ...
        quotes.push({
          ...
          providerInternalName: 'LogiHub Internacional',
          carrierName: 'Logihub',
...
app.post('/api/shipments/quote', async (req: any, res) => {
  try {
    const cacheKey = cacheService.getQuoteKey(req.body);
    ...
    const activeProviders = providersDb.filter((provider: any) => {
      ...
      return SHIP24GO_QUOTE_PROVIDER_CODES.has(code);
    });
    ...
    // Fresh FX rates once per request (used by EasyPost/LogiHub and final conversion)
    ...
    const providerResults = await Promise.allSettled(
      ...
      if (providerCode === 'parcelabc') {
        ...
        const startedAt = Date.now();
        try {
          const response = await fetch(`${providerBaseUrl(provider, 'https://www.parcelabc.com/api-pabc.php')}/quote`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify(requestPayload)
          });
      ...
      if (providerCode === 'paccofacile') {
        ...
        try {
          ...
          const pickupLocality = await lookupPaccofacileLocality(provider, keys, countryFrom, zipFrom, originCityHint);
          const destinationLocality = await lookupPaccofacileLocality(provider, keys, countryTo, zipTo, destCityHint);
          ...
          const { httpStatus, response: data } = await callPaccofacile(provider, keys, 'POST', 'service/shipment/quote', requestPayload);
      ...
      if (providerCode === 'spedirepro') {
        ...
        try {
          ...
          const startedAt = Date.now();
          const { httpStatus, response: data } = await callSpedirePro(provider, keys, 'POST', 'v1/get-quotes', requestPayload);
      ...
      if (providerCode === 'spediamopro') {
        ...
        try {
          ...
          const startedAt = Date.now();
          const { httpStatus, response: data } = await callSpediamoPro(provider, keys, 'POST', 'quotations', requestPayload);
      ...
      if (providerCode === 'easypost') {
        ...
        try {
          ...
          const startedAt = Date.now();
          const { httpStatus, response: data } = await callEasyPost(provider, keys, 'POST', 'shipments', requestPayload);
      ...
      if (providerCode === 'genei') {
        ...
        try {
          if (geneiApiMode(provider) === 'v1') {
            ...
            const { httpStatus, response: data } = await callGeneiV1(provider, keys, 'obtener_listado_agencias_precios', requestPayload);
            ...
            const bearerToken = await getGeneiToken(credential);
            const response = await fetch(`https://apiv2.genei.es/api/v2/agencies/prices?${params.toString()}`, {
              method: 'GET',
      ...
      // --- LogiHub Internacional (helper isolated) ---
    ...
    const allQuotesFx = allQuotes.map((q: any) => {
      try {
        // EasyPost / LogiHub already convert to requestedCurrency; still normalize fields.
    ...
    let activePlanName = 'Ship24Go Básico';
    ...
    if (userId) {
      try {
        ...
        if (planRows?.[0]) {
          ...
          activePlanName = String(planRows[0].plan_name || 'Ship24Go Básico');
          activePlanDiscountPercent = Math.max(0, Math.min(95, Number(planRows[0].discount_percent || 0)));
        }
      } catch (e) {
        activePlanDiscountPercent = 0;
        activePlanName = 'Ship24Go Básico';
      ...
      await pool.query(
        ...
        [
          ...
          JSON.stringify({
            ...
            planName: q.planName || 'Ship24Go Básico',
    ...
    try {
      cacheService.set(cacheKey, quotePayload, 300);
    } catch {}
...
app.post('/api/shipments', authMiddleware, async (req: any, res) => {
  try {
    ...
    const targetUserId = (isSuperAdmin && req.body.clientId) ? req.body.clientId : req.user.id;

    const [quoteRows]: any = await pool.query(
      `SELECT q.*, p.id AS db_provider_id, p.code AS db_provider_code, p.name AS db_provider_name
       FROM quotes q
       LEFT JOIN providers p ON p.id = q.provider_id
       WHERE q.id = ? AND (q.user_id = ? OR q.user_id IS NULL OR ? = 1)
       LIMIT 1`,
      [quoteId, req.user.id, isSuperAdmin ? 1 : 0]
    );
    ...
    const hasEnoughBalance = isSuperAdmin ? true : (userBalance >= cost);
    const providerManifest = requestedManifest && hasEnoughBalance ? 1 : 0;
    const canChargeWalletNow = !isSuperAdmin && requestedManifest && hasEnoughBalance;
    ...
    const newShipment = {
      ...
      label_base64: null,
      track_url: providerResult.trackUrl || (providerResult.trackingCode ? `${process.env.APP_URL || 'https://ship24go.com'}/track/${providerResult.trackingCode}` : null),
      payment_url: providerResult.paymentUrl || null,
      provider_payload_json: providerResult.providerPayload || null,
      walletDeduction,
    ...
    if (req.body.adminNotes) {
      await pool.query('UPDATE shipments SET admin_notes = ? WHERE id = ?', [req.body.adminNotes, shipmentId]).catch(() => null);
    }
    const adminCreatedAuditMsg = isSuperAdmin
      ? `Envío creado por Super Admin (${req.user.name || req.user.email})${req.body.clientId ? ' asignado a cliente' : ''}.`
      : (Number(manifest ?? 1) === 1 ? 'El envío fue creado correctamente.' : 'El envío fue preparado y queda pendiente de pago.');
    ...
    await TrackingEventRepo.create({
      ...
      status: providerResult.status,
      status_label: providerResult.statusLabel,
      description: adminCreatedAuditMsg
    });
...
app.post('/api/admin/easypost/webhook/create', authMiddleware, requireSuperAdmin, async (req: any, res) => {
  try {
    ...
    const cfg = providerConfig(provider || {});
    const webhookUrl = String(req.body?.url || cfg.webhookUrl || `${process.env.APP_URL || 'https://ship24go.com'}/api/webhooks/easypost`);
    ...
    const response = await fetch('https://api-m.sandbox.paypal.com/v2/checkout/orders', {
      ...
      body: JSON.stringify({
        ...
        purchase_units: [{
          ...
          description: 'Servicio de envío logístico Ship24go'
        }]
      })
    });
  ...
  try {
    ...
    if (!tokenOk) {
      ...
      if (!isOwner) {
        ...
        return res.send(`<!doctype html><html lang="es"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Etiqueta no disponible</title><style>body{font-family:Arial,sans-serif;background:#f8fafc;color:#0f172a;margin:0;display:grid;place-items:center;min-height:100vh}.card{background:#fff;border:1px solid #e2e8f0;border-radius:24px;box-shadow:0 20px 50px rgba(15,23,42,.08);max-width:520px;padding:32px;text-align:center}.btn{display:inline-block;margin-top:18px;background:#2563eb;color:white;text-decoration:none;font-weight:800;padding:12px 18px;border-radius:999px}</style></head><body><main class="card"><h1>Acceso protegido</h1><p>Para ver o descargar esta etiqueta, abre tu panel de Ship24Go y selecciona el envío correspondiente.</p><a class="btn" href="/panel/shipments">Ir a mis envíos</a></main></body></html>`);
      }
    }
...
app.get('/api/admin/roles', authMiddleware, requireAdminOrPermission('roles.view'), async (_req: any, res) => {
  try {
...
app.post('/api/admin/roles', authMiddleware, requireAdminOrPermission('roles.manage'), async (req: any, res) => {
  try {
    ...
    res.json({ role, message: 'Rol creado exitosamente.' });
  } catch (error) {
...
app.put('/api/admin/roles/:id', authMiddleware, requireAdminOrPermission('roles.manage'), async (req: any, res) => {
  try {
    const { name, description, permissions } = req.body;
    ...
    res.json({ role, message: 'Rol actualizado exitosamente.' });
  } catch (error) {
...
app.delete('/api/admin/roles/:id', authMiddleware, requireAdminOrPermission('roles.manage'), async (req: any, res) => {
  try {
    const deleted = await RoleRepo.delete(req.params.id);
    if (!deleted) {
      return res.status(400).json({ error: 'No se puede eliminar un rol protegido del sistema.' });
    }
...
app.get('/api/admin/team', authMiddleware, requireAdminOrPermission('team.view'), async (_req: any, res) => {
  try {
    const members = await TeamRepo.getMembers();
    const roles = await RoleRepo.getAll();
...
app.delete('/api/admin/team/:id', authMiddleware, requireAdminOrPermission('team.manage'), async (req: any, res) => {
  try {
    const deleted = await TeamRepo.deleteMember(req.params.id);
    if (!deleted) {
      return res.status(400).json({ error: 'No se puede eliminar el usuario Super Administrador principal.' });
    }
    res.json({ success: true, message: 'Integrante eliminado exitosamente.' });
  } catch (error) {
    console.error('Error deleting team member:', error);
    res.status(500).json({ error: 'No se pudo eliminar el integrante.' });
  }
});
...
// Admin: Envíos con auditoría de creadores, hojas de ruta y eventos
app.get('/api/admin/shipments', authMiddleware, requireSuperAdmin, async (req: any, res) => {
  try {
    ...
    // Obtener información detallada de los creadores de los envíos
    const userMap = new Map<string, any>();
    if (shipmentsList.length > 0) {
      const userIds = [...new Set(shipmentsList.map((s: any) => s.user_id).filter(Boolean))];
      if (userIds.length > 0) {
        const placeholders = userIds.map(() => '?').join(',');
        try {
          const [userRows]: any = await pool.query(
            `SELECT u.id, u.name, u.email, u.phone, u.role, u.role_id, u.business_type, u.country, r.name as role_name 
             FROM users u 
          ...
          console.error('Error fetching shipment creator users:', e);
        }
      }
    }

    // Obtener historial de eventos y logs de seguimiento para cada envío
    const eventsMap = new Map<string, any[]>();
    if (shipmentsList.length > 0) {
      const shipmentIds = shipmentsList.map((s: any) => s.id);
      const placeholders = shipmentIds.map(() => '?').join(',');
      try {
        const [eventRows]: any = await pool.query(
          `SELECT id, shipment_id, tracking_code, status_code, status_label, description, location, event_time, created_at 
        ...
        for (const ev of eventRows) {
          const list = eventsMap.get(ev.shipment_id) || [];
          list.push({
            id: ev.id,
            shipmentId: ev.shipment_id,
            trackingCode: ev.tracking_code,
          ...
          eventsMap.set(ev.shipment_id, list);
        }
      } catch (e) {
        console.error('Error fetching tracking events for admin shipments:', e);
      }
    }
    ...
    const normalized = shipmentsList.map(s => {
      ...
      const creator = userMap.get(s.user_id) || {
        id: s.user_id,
      ...
      return ({
        ...
        creator: {
          id: creator.id,
          name: creator.name || 'Sin nombre',
          email: creator.email || '',
          phone: creator.phone || '',
          role: creator.role || 'customer',
          roleName: creator.role_name || (creator.role === 'super_admin' ? 'Super Admin' : creator.role === 'admin' ? 'Administrador' : 'Cliente'),
          businessType: creator.business_type || '',
        ...
        routeSheetUrl: s.route_sheet_url || null,
        routeSheetName: s.route_sheet_name || null,
        adminNotes: s.admin_notes || '',
        isArchived: Boolean(s.is_archived),
        archivedAt: s.archived_at || null,
        events: eventsMap.get(s.id) || []
      });
    });
    res.json({ shipments: normalized });
  } catch (error) {
    console.error('Error en GET /api/admin/shipments:', error);
    res.status(500).json({ error: 'No se pudo completar la operación.' });
  }
});
...
app.post('/api/admin/shipments/:id/status', authMiddleware, requireSuperAdmin, async (req: any, res) => {
  try {
    ...
    const adminIdentifier = req.user?.name ? `${req.user.name} (${req.user.email})` : (req.user?.email || 'Super Admin');
    await TrackingEventRepo.create({
      shipment_id: shipment.id,
      tracking_code: shipment.tracking_code,
      status: status.toLowerCase(),
      status_label: status,
      description: `Estado cambiado a "${status}" por ${adminIdentifier}`
    });
...
// Admin: Edición Completa del Envío con Registro de Auditoría
app.put('/api/admin/shipments/:id', authMiddleware, requireSuperAdmin, async (req: any, res) => {
  try {
    const shipment = await ShipmentRepo.getById(req.params.id);
    if (!shipment) return res.status(404).json({ error: 'Envío no encontrado' });
    ...
    if (status && status !== shipment.status) {
      ...
      values.push(statusLabel || status);
      auditChanges.push(`Estado cambiado a "${statusLabel || status}"`);
    } else if (statusLabel && statusLabel !== shipment.status_label) {
      updates.push('status_label = ?');
      values.push(statusLabel);
      auditChanges.push(`Etiqueta de estado cambiada a "${statusLabel}"`);
    }

    if (providerTrackingCode !== undefined && providerTrackingCode !== shipment.provider_tracking_code) {
      updates.push('provider_tracking_code = ?');
      values.push(providerTrackingCode || null);
      auditChanges.push(`Tracking de transportista: "${providerTrackingCode || 'Ninguno'}"`);
    }

    if (trackingCode !== undefined && trackingCode !== shipment.tracking_code && trackingCode.trim()) {
      updates.push('tracking_code = ?');
      values.push(trackingCode.trim());
      auditChanges.push(`Tracking interno: "${trackingCode.trim()}"`);
    }

    if (adminNotes !== undefined && adminNotes !== shipment.admin_notes) {
      updates.push('admin_notes = ?');
      values.push(adminNotes || null);
      auditChanges.push(`Notas internas actualizadas`);
    }

    if (recipient && typeof recipient === 'object') {
      const prevRecipient = typeof shipment.recipient_json === 'string' ? JSON.parse(shipment.recipient_json) : (shipment.recipient_json || {});
      const mergedRecipient = { ...prevRecipient, ...recipient };
      updates.push('recipient_json = ?');
      values.push(JSON.stringify(mergedRecipient));
      auditChanges.push(`Destinatario modificado (${mergedRecipient.name || mergedRecipient.fullName || ''})`);
    }

    if (carrierName) {
      const payload = parseJsonSafe(shipment.provider_payload_json) || {};
      payload.carrierName = carrierName;
      updates.push('provider_payload_json = ?');
      values.push(JSON.stringify(payload));
      auditChanges.push(`Transportista asignado: "${carrierName}"`);
    }

    if (updates.length > 0) {
      updates.push('updated_at = NOW()');
      values.push(shipment.id);
      await pool.query(`UPDATE shipments SET ${updates.join(', ')} WHERE id = ?`, values);
    }

    const adminIdentifier = req.user?.name ? `${req.user.name} (${req.user.email})` : (req.user?.email || 'Super Admin');
    const description = auditChanges.length > 0
      ? `Modificado por ${adminIdentifier}: ${auditChanges.join('; ')}`
      : `Envío actualizado por ${adminIdentifier}`;

    await TrackingEventRepo.create({
      shipment_id: shipment.id,
      tracking_code: trackingCode || shipment.tracking_code,
      status: (status || shipment.status).toLowerCase(),
      status_label: statusLabel || status || shipment.status_label,
      description
    });

    const updatedShipment = await ShipmentRepo.getById(shipment.id);
    res.json({ success: true, message: 'Envío actualizado correctamente', shipment: updatedShipment });
  } catch (error: any) {
    console.error('Error actualizando envío:', error);
    res.status(500).json({ error: error?.message || 'No se pudo actualizar el envío.' });
  }
});

// Admin: Subir Hoja de Ruta (Route Sheet / Waybill)
app.post('/api/admin/shipments/:id/route-sheet', authMiddleware, requireSuperAdmin, async (req: any, res) => {
  try {
    const shipment = await ShipmentRepo.getById(req.params.id);
    if (!shipment) return res.status(404).json({ error: 'Envío no encontrado' });
    ...
    if (!fileData || !fileName) {
      return res.status(400).json({ error: 'Se requiere el archivo de hoja de ruta.' });
    }

    const matches = fileData.match(/^data:([A-Za-z-+\/]+);base64,(.+)$/);
    if (!matches || matches.length !== 3) {
      return res.status(400).json({ error: 'Formato de archivo inválido. Debe ser PDF o imagen.' });
    }
    ...
    if (buffer.length > 15 * 1024 * 1024) {
      return res.status(400).json({ error: 'El archivo no puede exceder los 15MB.' });
    }
    ...
    const cleanBaseName = String(fileName).replace(/[^a-zA-Z0-9_\-\.]/g, '_').replace(/\.[^/.]+$/, '');
    const uniqueFilename = `hoja-ruta-${shipment.tracking_code}-${Date.now()}.${ext}`;

    const publicUploadDir = path.join(process.cwd(), 'public', 'uploads', 'route-sheets');
    const distUploadDir = path.join(process.cwd(), 'dist', 'uploads', 'route-sheets');

    fs.mkdirSync(publicUploadDir, { recursive: true });
    fs.mkdirSync(distUploadDir, { recursive: true });

    const publicPath = path.join(publicUploadDir, uniqueFilename);
    const distPath = path.join(distUploadDir, uniqueFilename);

    fs.writeFileSync(publicPath, buffer);
    try { fs.writeFileSync(distPath, buffer); } catch {}

    const routeSheetUrl = `/uploads/route-sheets/${uniqueFilename}`;
    const cleanFileName = fileName.slice(0, 200);

    await pool.query(
      'UPDATE shipments SET route_sheet_url = ?, route_sheet_name = ?, updated_at = NOW() WHERE id = ?',
      [routeSheetUrl, cleanFileName, shipment.id]
    );
    ...
    await TrackingEventRepo.create({
      ...
      description: `Hoja de ruta cargada por ${adminIdentifier}: ${cleanFileName}`
    });

    res.json({
      success: true,
      message: 'Hoja de ruta cargada exitosamente.',
      routeSheetUrl,
      routeSheetName: cleanFileName
    });
  } catch (error: any) {
    console.error('Error subiendo hoja de ruta:', error);
    res.status(500).json({ error: error?.message || 'Error al subir la hoja de ruta.' });
  }
});

// Admin: Eliminar Hoja de Ruta
app.delete('/api/admin/shipments/:id/route-sheet', authMiddleware, requireSuperAdmin, async (req: any, res) => {
  try {
    const shipment = await ShipmentRepo.getById(req.params.id);
    if (!shipment) return res.status(404).json({ error: 'Envío no encontrado' });

    const prevSheetName = shipment.route_sheet_name || 'Hoja de ruta';

    if (shipment.route_sheet_url) {
      const filename = path.basename(shipment.route_sheet_url);
      const publicPath = path.join(process.cwd(), 'public', 'uploads', 'route-sheets', filename);
      const distPath = path.join(process.cwd(), 'dist', 'uploads', 'route-sheets', filename);
      try { if (fs.existsSync(publicPath)) fs.unlinkSync(publicPath); } catch {}
      try { if (fs.existsSync(distPath)) fs.unlinkSync(distPath); } catch {}
    }

    await pool.query(
      'UPDATE shipments SET route_sheet_url = NULL, route_sheet_name = NULL, updated_at = NOW() WHERE id = ?',
      [shipment.id]
    );
    ...
    await TrackingEventRepo.create({
      ...
      status: shipment.status,
      status_label: shipment.status_label,
      description: `Hoja de ruta eliminada por ${adminIdentifier} (Archivo anterior: ${prevSheetName})`
    });

    res.json({ success: true, message: 'Hoja de ruta eliminada correctamente.' });
  } catch (error: any) {
    console.error('Error eliminando hoja de ruta:', error);
    res.status(500).json({ error: error?.message || 'Error al eliminar la hoja de ruta.' });
  }
});

// Admin: Archivar o Desarchivar Envío
app.put('/api/admin/shipments/:id/archive', authMiddleware, requireSuperAdmin, async (req: any, res) => {
  try {
    const shipment = await ShipmentRepo.getById(req.params.id);
    if (!shipment) return res.status(404).json({ error: 'Envío no encontrado' });

    const shouldArchive = req.body.isArchived !== undefined ? Boolean(req.body.isArchived) : !Boolean(shipment.is_archived);
    const archivedAt = shouldArchive ? new Date() : null;

    await pool.query(
      'UPDATE shipments SET is_archived = ?, archived_at = ?, updated_at = NOW() WHERE id = ?',
      [shouldArchive ? 1 : 0, archivedAt, shipment.id]
    );
    ...
    await TrackingEventRepo.create({
      ...
      description: shouldArchive
        ? `[Auditoría] Envío archivado por ${adminIdentifier}`
        : `[Auditoría] Envío desarchivado y restaurado a la lista activa por ${adminIdentifier}`
    });

    auditService.log({
      req,
      action: shouldArchive ? 'SHIPMENT_ARCHIVE' : 'SHIPMENT_UNARCHIVE',
      entityType: 'shipment',
      entityId: shipment.id,
      details: { trackingCode: shipment.tracking_code, shouldArchive, customerId: shipment.user_id }
    });

    res.json({ 
      success: true, 
      isArchived: shouldArchive, 
      message: shouldArchive ? 'Envío archivado correctamente.' : 'Envío desarchivado y restaurado a la lista activa.' 
    });
  } catch (error: any) {
    console.error('Error al archivar/desarchivar envío:', error);
    res.status(500).json({ error: error?.message || 'No se pudo cambiar el estado de archivo.' });
  }
});

// Admin: Eliminar Envío Permanentemente
app.delete('/api/admin/shipments/:id', authMiddleware, requireSuperAdmin, async (req: any, res) => {
  try {
    const shipment = await ShipmentRepo.getById(req.params.id);
    if (!shipment) return res.status(404).json({ error: 'Envío no encontrado' });

    // Si tiene hoja de ruta física, borrarla del disco
    if (shipment.route_sheet_url) {
      const filename = path.basename(shipment.route_sheet_url);
      const publicPath = path.join(process.cwd(), 'public', 'uploads', 'route-sheets', filename);
      const distPath = path.join(process.cwd(), 'dist', 'uploads', 'route-sheets', filename);
    ...
    // Borrar paquetes y eventos asociados
    try { await pool.query('DELETE FROM shipment_packages WHERE shipment_id = ?', [shipment.id]); } catch {}
    try { await pool.query('DELETE FROM tracking_events WHERE shipment_id = ?', [shipment.id]); } catch {}
    
    // Borrar el envío de la base de datos
    await pool.query('DELETE FROM shipments WHERE id = ?', [shipment.id]);

    const adminIdentifier = req.user?.name ? `${req.user.name} (${req.user.email})` : (req.user?.email || 'Super Admin');
    console.log(`[Auditoría Super Admin] Envío ${shipment.tracking_code} (${shipment.id}) eliminado permanentemente por ${adminIdentifier}`);
    auditService.log({
      req,
      action: 'SHIPMENT_DELETE',
      entityType: 'shipment',
      entityId: shipment.id,
      details: { trackingCode: shipment.tracking_code, customerId: shipment.user_id }
    });

    res.json({ success: true, message: `Envío ${shipment.tracking_code} eliminado permanentemente.` });
  } catch (error: any) {
    console.error('Error al eliminar envío:', error);
    res.status(500).json({ error: error?.message || 'No se pudo eliminar el envío.' });
  }
});

// Admin: Archivar o Desarchivar Envíos en Lote (Bulk Archive)
app.post('/api/admin/shipments/bulk/archive', authMiddleware, requireSuperAdmin, async (req: any, res) => {
  try {
    ...
    const shouldArchive = isArchived !== undefined ? Boolean(isArchived) : true;
    const archivedAt = shouldArchive ? new Date() : null;
    const placeholders = ids.map(() => '?').join(',');

    await pool.query(
      `UPDATE shipments SET is_archived = ?, archived_at = ?, updated_at = NOW() WHERE id IN (${placeholders})`,
      [shouldArchive ? 1 : 0, archivedAt, ...ids]
    );

    const adminIdentifier = req.user?.name ? `${req.user.name} (${req.user.email})` : (req.user?.email || 'Super Admin');
    
    // Obtener tracking_codes para registrar auditoría
    const [rows]: any = await pool.query(`SELECT id, tracking_code, status, status_label FROM shipments WHERE id IN (${placeholders})`, ids);
    for (const row of rows) {
      await TrackingEventRepo.create({
        shipment_id: row.id,
        ...
        description: shouldArchive
          ? `[Auditoría en Lote] Envío archivado por ${adminIdentifier}`
          : `[Auditoría en Lote] Envío restaurado a la lista activa por ${adminIdentifier}`
      }).catch(() => null);
    }

    auditService.log({
      req,
      action: shouldArchive ? 'SHIPMENT_BULK_ARCHIVE' : 'SHIPMENT_BULK_UNARCHIVE',
      entityType: 'shipment_batch',
      details: { count: ids.length, isArchived: shouldArchive, sampleIds: ids.slice(0, 20) }
    });
    ...
    res.json({
      ...
      message: shouldArchive
        ? `${ids.length} envíos archivados correctamente.`
        : `${ids.length} envíos restaurados a la lista activa.`
    });
  } catch (error: any) {
    console.error('Error en bulk archive:', error);
    res.status(500).json({ error: error?.message || 'No se pudo archivar en lote.' });
  }
});

// Admin: Eliminar Envíos en Lote Permanentemente (Bulk Delete)
app.post('/api/admin/shipments/bulk/delete', authMiddleware, requireSuperAdmin, async (req: any, res) => {
  try {
    const { ids } = req.body;
    if (!Array.isArray(ids) || ids.length === 0) {
      return res.status(400).json({ error: 'Lista de IDs no válida o vacía' });
    ...
    // Obtener hojas de ruta si las hay para borrarlas del disco
    const [rows]: any = await pool.query(`SELECT id, tracking_code, route_sheet_url FROM shipments WHERE id IN (${placeholders})`, ids);
    for (const row of rows) {
      if (row.route_sheet_url) {
        const filename = path.basename(row.route_sheet_url);
        const publicPath = path.join(process.cwd(), 'public', 'uploads', 'route-sheets', filename);
        const distPath = path.join(process.cwd(), 'dist', 'uploads', 'route-sheets', filename);
        try { if (fs.existsSync(publicPath)) fs.unlinkSync(publicPath); } catch {}
        try { if (fs.existsSync(distPath)) fs.unlinkSync(distPath); } catch {}
    ...
    try { await pool.query(`DELETE FROM shipment_packages WHERE shipment_id IN (${placeholders})`, ids); } catch {}
    try { await pool.query(`DELETE FROM tracking_events WHERE shipment_id IN (${placeholders})`, ids); } catch {}
    await pool.query(`DELETE FROM shipments WHERE id IN (${placeholders})`, ids);

    const adminIdentifier = req.user?.name ? `${req.user.name} (${req.user.email})` : (req.user?.email || 'Super Admin');
    console.log(`[Auditoría Super Admin] ${ids.length} envíos eliminados permanentemente por ${adminIdentifier}`);
    auditService.log({
      req,
      action: 'SHIPMENT_BULK_DELETE',
      entityType: 'shipment_batch',
      details: { count: ids.length, sampleIds: ids.slice(0, 20) }
    });

    res.json({ success: true, count: ids.length, message: `${ids.length} envíos eliminados permanentemente.` });
  } catch (error: any) {
    console.error('Error en bulk delete:', error);
    res.status(500).json({ error: error?.message || 'No se pudo eliminar en lote.' });
  }
});

// Admin: Cambiar Estado en Lote (Bulk Status Update)
app.post('/api/admin/shipments/bulk/status', authMiddleware, requireSuperAdmin, async (req: any, res) => {
  try {
    const { ids, status, statusLabel } = req.body;
    if (!Array.isArray(ids) || ids.length === 0 || !status) {
      return res.status(400).json({ error: 'Parámetros inválidos para actualización en lote' });
    }
    ...
    await pool.query(
      `UPDATE shipments SET status = ?, status_label = ?, updated_at = NOW() WHERE id IN (${placeholders})`,
      [status, label, ...ids]
    );

    const adminIdentifier = req.user?.name ? `${req.user.name} (${req.user.email})` : (req.user?.email || 'Super Admin');
    const [rows]: any = await pool.query(`SELECT id, tracking_code FROM shipments WHERE id IN (${placeholders})`, ids);
    for (const row of rows) {
      await TrackingEventRepo.create({
        shipment_id: row.id,
        tracking_code: row.tracking_code,
        status,
        status_label: label,
        description: `[Auditoría en Lote] Estado cambiado a '${label}' por ${adminIdentifier}`
      }).catch(() => null);
    }

    res.json({ success: true, count: ids.length, message: `Estado actualizado para ${ids.length} envíos.` });
  } catch (error: any) {
    console.error('Error en bulk status:', error);
    res.status(500).json({ error: error?.message || 'No se pudo actualizar el estado en lote.' });
  }
});
...
app.post('/api/admin/polar/webhook/create', authMiddleware, requireSuperAdmin, async (_req: any, res) => {
  try {
    ...
    const payload = {
      ...
      format: 'raw',
      enabled: true,
      name: 'Ship24Go Wallet y Suscripciones',
      events: [
        'checkout.created',
        'checkout.updated',
        'order.paid',
...
app.post('/api/admin/polar/products/sync', authMiddleware, requireSuperAdmin, async (_req: any, res) => {
  try {
    ...
    if (!plansRows.length) {
      return res.status(400).json({ error: 'No hay planes internos de Ship24Go para publicar.' });
    }
    ...
    function cleanName(name: string) {
      return String(name || 'Ship24Go Plan').trim().slice(0, 64);
    }
    ...
    async function createOrLoadPlanProduct(plan: any) {
      ...
      if (!data?.id) {
        const payload = {
          ...
          description: `Ship24Go ${plan.name} - plan de suscripción`,
          visibility: 'public',
    ...
    if (!walletProductId) {
      const walletPayload: any = {
        name: 'Ship24Go Wallet Recarga',
        description: 'Producto interno para recargas variables de saldo Ship24Go.',
        visibility: 'public',
        prices: [
    ...
    await pool.query(
      `UPDATE providers
       SET is_active = 1,
           ...
           last_tested_at = NOW()
       WHERE code = 'polar'`,
      [failed.length ? 'warning' : 'ok', failed.length ? 'Sincronización parcial de planes Polar' : 'Planes Ship24Go publicados en Polar']
    );
    ...
    res.json({
      ...
      message: failed.length
        ...
        : `Planes Ship24Go sincronizados correctamente.`,
      walletProductId,
      walletStatus,
      walletError,
    ...
    res.status(500).json({ error: 'No se pudieron publicar los planes Ship24Go en Polar.' });
  }
});
  ...
  try {
    ...
    res.json({
      ...
      events: [
        ...
        { code: 'shipment_updated', title: 'Aggiornamenti spedizione', description: 'Actualiza el estado público del tracking en Ship24Go.' },
...
app.post('/api/admin/email/test', authMiddleware, requireSuperAdmin, async (req: any, res) => {
  try {
    ...
    const result = await sendShipmentStatusEmail(fakeShipment, 'tramitado', lang === 'it' ? 'Spedizione elaborata' : lang === 'en' ? 'Shipment processed' : lang === 'fr' ? 'Envoi traité' : 'Envío tramitado', 'Mensaje de prueba de notificación Ship24Go.');
    ...
    for (const plan of plans) {
      ...
      if (!productId) {
        const productResponse = await fetch(`${base}/v1/catalogs/products`, {
          ...
          body: JSON.stringify({ name: `Ship24Go ${plan.name}`, type: 'SERVICE', category: 'SHIPPING_AND_DELIVERY', description: `Plan ${plan.name} de Ship24Go` })
        });
...
app.post('/api/admin/plans', authMiddleware, requireSuperAdmin, async (req: any, res) => {
  try {
    await PlanRepo.updateAll(req.body.plans);
    const plans = await PlanRepo.getAll();
...
Write a professional logistics support reply for Ship24Go. Use the customer's language (${language}). Use the word courier for shipping partners. Do not expose internal systems, raw data, keys, logs, tables or technical errors. Keep it friendly and concise.`;
...
const copilotMessages: Record<string, Record<string, string>> = {
  es: {
    ...
    handoffReply: 'Gracias por tu mensaje. Un agente de Ship24Go continuará la asistencia desde este ticket.'
  },
  en: {
    ...
    handoffReply: 'Thank you for your message. A Ship24Go agent will continue the assistance from this ticket.'
  },
  it: {
    ...
    handoffReply: 'Grazie per il tuo messaggio. Un agente Ship24Go continuerà l’assistenza da questo ticket.'
  },
  fr: {
    ...
    handoffReply: 'Merci pour votre message. Un agent Ship24Go poursuivra l’assistance depuis ce ticket.'
  },
  de: {
    ...
    handoffReply: 'Vielen Dank für Ihre Nachricht. Ein Ship24Go Mitarbeiter setzt die Unterstützung über dieses Ticket fort.'
  },
  zh: {
    ...
    category: '人工协助',
    summary: 'AI 助手对话摘要',
    handoffReply: '感谢您的留言。Ship24Go 客服人员将通过此工单继续协助您。'
  }
};
...
function ship24goSupportSystemPrompt(lang: string): string {
  return `You are Ship24Go Copilot, a professional logistics support assistant.
Respond in ${COPILOT_LANGUAGES[normalizeCopilotLanguage(lang)] || 'Spanish'}.
Use only the context provided by Ship24Go backend. Do not invent shipment status, invoices, balances, labels, provider responses or internal facts.
...
app.get('/membership', (_req, res) => {
  res.setHeader('Cache-Control', 'public, max-age=300');
  res.sendFile('/www/wwwroot/ship24go.com/public/membership.html');
});
