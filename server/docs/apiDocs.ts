/**
 * Ship24Go Customer API Documentation (PUBLIC SAFE)
 ...
 * Internal aggregation is presented only as "Ship24Go network".
 */
...
const BASE = 'https://ship24go.com/api';
const APP = 'https://ship24go.com';
const VER = '1.3.0';
...
function openApiDescription(): string {
  return [
    "# Ship24Go multi-carrier shipping API",
    "",
    ...
    "",
    "## Useful links",
    "- PDF guide (EN): https://ship24go.com/api/docs/pdf?lang=en",
    "- PDF guide (ES): https://ship24go.com/api/docs/pdf?lang=es",
    "- OpenAI tools: https://ship24go.com/api/docs/openai-tools.json",
    "- Customer panel: https://ship24go.com/panel/api-docs",
    "",
...
export function getOpenApiSpec() {
  return {
    openapi: '3.0.3',
    info: {
      title: 'Ship24Go Shipping API',
      version: VER,
      description: openApiDescription(),
      contact: { name: 'Ship24Go Support', url: APP, email: 'support@ship24go.com' },
      license: { name: 'Proprietary' },
    },
    externalDocs: {
      description: 'Ship24Go website',
      url: APP,
    },
    tags: [
          ...
          properties: {
            ...
            carrierName: {
              ...
              enum: [
                ...
                'Chronopost', 'Colissimo', 'Hermes', 'DPD', 'Logihub',
              ],
            },
            provider: { type: 'string', example: 'Logihub', description: 'Ship24Go public network brand' },
...
export function getOpenAiToolSchemas() {
  return {
    ...
    system_prompt:
      'You are a Ship24Go multi-carrier shipping assistant. Quote carriers (FedEx, UPS, DHL, Correos, BRT, InPost, Poste Italiane, etc.), compare price and ETA, and create shipments only after the user confirms. Never invent rates or tracking numbers. Present carrierName to the user — never invent wholesale supplier names.',
    base_url: BASE,
    ...
    http_mapping: {
      ship24go_quote: { method: 'POST', path: '/shipments/quote' },
      ship24go_create_shipment: { method: 'POST', path: '/shipments' },
      ship24go_track: { method: 'GET', path: '/tracking/{code}' },
      ship24go_dropoff_points: { method: 'POST', path: '/drop-off-points' },
      ship24go_list_shipments: { method: 'GET', path: '/shipments' },
    },
  };
}
  ...
  sections: [
    {
      ...
      body: `Ship24Go is a unified multi-carrier shipping platform for ecommerce and logistics teams.
...
OTHER / NETWORK
- Logihub (Ship24Go public network brand when a service is sold under network branding)

DO NOT show, log, or document wholesale purchase channels, aggregator vendor codes, or supplier margins to end users.
Ship24Go routes each shipment through its private logistics network. That routing is internal.
...
Do not expose internal routing metadata to shoppers.`,
    },
    {
      ...
      body: `Use your OpenAI key + Ship24Go tools:

1) Your backend stores the Ship24Go JWT
2) GET ${BASE}/docs/openai-tools.json
3) Map tool calls to Ship24Go REST
4) Show carrierName to the end user

Tools: ship24go_quote, ship24go_create_shipment, ship24go_track,
ship24go_dropoff_points, ship24go_list_shipments
...
# PDF
curl -sL "${BASE}/docs/pdf?lang=en" -o Ship24Go-API-EN.pdf`,
    },
  ],
};
  ...
  sections: [
    {
      ...
      body: `Ship24Go es una plataforma unificada multi-transportista.
...
RED
- Logihub (marca pública de red Ship24Go cuando aplica)

NO muestres canales de compra mayorista, códigos de agregador internos ni márgenes de suministro.
Ship24Go enruta por su red privada: eso es interno.
...
const IT: DocBundle = {
  ...
  sections: [
    {
      title: '1. Panoramica',
      body: `Ship24Go unifica più corrieri in una sola API.
Base URL: ${BASE}
...
BRT, InPost, SDA, Poste Italiane, GLS,
Logihub (brand rete pubblica Ship24Go quando applicabile)

NON esporre canali di acquisto all'ingrosso, codici fornitore interni o margini di rete.
Ship24Go instrada in modo privato: resta interno.
...
export function docsToPdfBuffer(doc: DocBundle): Buffer {
  ...
  drawText(`Version ${doc.version}  |  ${doc.updated}  |  ${doc.lang.toUpperCase()}  |  ship24go.com`, 8.5, false);
  y -= 6;
